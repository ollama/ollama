...
func convertFull(t *testing.T, fsys fs.FS) (*os.File, modelfile.KV, modelfile.Tensors) {
	t.Helper()
	...
	m, _, err := modelfile.DecodeGGML(r, math.MaxInt)
	if err != nil {
		t.Fatal(err)
	}
...
func generateResultsJSON(t *testing.T, f *os.File, kv modelfile.KV, tensors modelfile.Tensors) map[string]string {
