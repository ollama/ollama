...
func (m *gptossModel) KV(t *Tokenizer) ggml.KV {
	...
	kv["embedding_length"] = m.HiddenSize
	kv["feed_forward_length"] = m.IntermediateSize
	kv["expert_count"] = cmp.Or(m.Experts, m.LocalExperts)
	kv["expert_used_count"] = m.ExpertsPerToken
	kv["attention.head_count"] = m.AttentionHeads
	kv["attention.head_count_kv"] = m.KeyValueHeads
	kv["attention.key_length"] = m.HeadDim
	kv["attention.value_length"] = m.HeadDim
	kv["attention.layer_norm_rms_epsilon"] = cmp.Or(m.RMSNormEpsilon, 1e-5)
	kv["attention.sliding_window"] = m.SlidingWindow
	kv["rope.freq_base"] = m.RopeTheta
	kv["rope.scaling.factor"] = cmp.Or(m.RopeScalingFactor, m.RopeScaling.Factor)
	kv["rope.scaling.original_context_length"] = m.InitialContextLength
