...
func (q *qwen35Model) parseMore(_ fs.FS) error {
	...
	if q.IsMoE {
		if q.NumExperts == 0 {
			return fmt.Errorf("qwen35: num_experts must be set for MoE models")
		}
	} else if q.NumExperts > 0 {
		return fmt.Errorf("qwen35: dense model should not set num_experts")
	}
...
func (w concatWriter) WriteTo(out io.Writer) (int64, error) {
	if _, err := w.first.WriteTo(out); err != nil {
		return 0, err
	}
	if _, err := w.second.WriteTo(out); err != nil {
		return 0, err
	}
	return 0, nil
}
