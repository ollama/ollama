...
func buildLloydMaxCodebook(dim int, bits int) []float32 {
	...
	// 65536 = 2^16 samples per (dim, bits) codebook. Sized so that the
	// per-quantile sample count (samples/levels) stays comfortably above the
	// noise floor for the largest level count we use: at 4-bit codebooks
	// (16 levels) that's 4096 samples per centroid, well above the central-
	// limit threshold for stable mean estimates of the marginal coord
	// distribution. Codebooks are cached per (dim, bits) at first use, so
	// the construction cost is paid once per process.
