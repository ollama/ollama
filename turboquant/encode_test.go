...
func TestMemoryFormulaMatchesMarshalSize(t *testing.T) {
	...
	for _, tc := range cases {
		...
		// 29 bytes per-block fixed header (×2).
		wantKey := 76 + bitmap + outlierData + ((dim-outlierCount)*regularKeyBits+7)/8
		wantValue := 76 + bitmap + outlierData + ((dim-outlierCount)*regularValueBits+7)/8
...
// TestEncodeKeyAndValueDistinct verifies that EncodeKeyVector and
// EncodeValueVector are independently invocable and produce blocks with the
// expected bit width per role (K uses KeyPrimaryBits; V uses ValueBits).
func TestEncodeKeyAndValueDistinct(t *testing.T) {
	values := pseudoRandomVector(32, 0x77)
	// Distinct K-bit vs V-bit widths so the wire encodes them differently.
	p := newPreset(100, "k3_v4", 3, 4, 0x35c0ffee, 4, 0)
	keyEncoded, err := EncodeKeyVector(values, p)
	if err != nil {
		t.Fatal(err)
	}
	valueEncoded, err := EncodeValueVector(values, p)
	if err != nil {
		t.Fatal(err)
	}
	if keyEncoded.Blocks[0].RegularBits != uint8(p.KeyPrimaryBits) {
		t.Fatalf("key RegularBits=%d, want %d", keyEncoded.Blocks[0].RegularBits, p.KeyPrimaryBits)
	}
	if valueEncoded.Blocks[0].RegularBits != uint8(p.ValueBits) {
		t.Fatalf("value RegularBits=%d, want %d", valueEncoded.Blocks[0].RegularBits, p.ValueBits)
	}
}
...
func TestPresetNames(t *testing.T) {
	...
	// Plausible-looking variants that are not part of the ship matrix must
	// not resolve. Ablation variants are reachable through env vars
	// (OLLAMA_TQ_DISABLE_OUTLIERS, OLLAMA_TQ_DISABLE_ASYMMETRIC).
	for _, name := range []string{
		...
		if _, err := PresetByName(name); err == nil {
			t.Errorf("non-ship preset %q must not resolve via PresetByName", name)
		}
	}
}
	...
	for _, preset := range []Preset{testOutlierPreset(PresetTQ2, 32), testOutlierPreset(PresetTQ3, 32)} {
		t.Run(preset.Name, func(t *testing.T) {
			...
			for range trials {
				...
				unifBlock, err := encodeSubBlock(vec, nil, preset, uniformBits, preset.RotationSeed)
				if err != nil {
					t.Fatal(err)
				}
...
func TestEncodeKeyPerHeadRoundTrip(t *testing.T) {
	...
	for _, preset := range []Preset{PresetTQ2, PresetTQ3} {
		t.Run(preset.Name, func(t *testing.T) {
			...
			// Uniform encoding (no outlier split) has higher error than the
			// full preset matrix. These thresholds validate the codec works,
			// not paper-quality reconstruction.
			maxRelErr := 0.45
...
// (gemma3 global = D=256, gemma4 global = D=512). Used as a per-D oracle
// for the GPU kernels — kernel divergence at a given D against this
// reference is a kernel-side issue.
