...
func (u *Updater) stopChecker(t *testing.T, cancel context.CancelFunc) {
	t.Helper()
	cancel()
	if u.checkerDone == nil {
		return
	}
	select {
	case <-u.checkerDone:
...
func TestBackgroundCheckerSkipsAlreadyStagedETagDownload(t *testing.T) {
	...
	t.Cleanup(func() { updater.stopChecker(t, cancel) })
...
func TestAutoUpdateReenabledDownloadsUpdate(t *testing.T) {
	...
	t.Cleanup(func() { upd.stopChecker(t, cancel) })
