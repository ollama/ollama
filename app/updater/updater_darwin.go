...
func DoUpgrade(interactive bool) error {
	...
	f.Close()
	// Make sure to remove the staged download now that we succeeded so we don't inadvertantly try again.
...
// If we detect an upgrade bundle, attempt to ugprade at startup
