...
const (
	...
	// cache-ready contains the signed payload's version so a replacement app
	// can reject staging left behind by an older installation.
	stagedCacheReadyFile           = "cache-ready"
	stagedCacheVersionFile         = "cache-version"
...
func (u *Updater) downloadInstallScriptRelease(ctx context.Context, updateResp UpdateResponse) error {
	...
	requestedVersion := normalizedSemver(normalizedUpdateVersion(updateResp))
	if strings.TrimSpace(updateResp.UpdateVersion) != "" && requestedVersion == "" {
		return fmt.Errorf("invalid update version %q", updateResp.UpdateVersion)
	}
	...
	if !downloadedScript && isInstallScriptStageReady(scriptStageDir, requestedVersion) {
	...
	targetVersion, err := runInstallScriptCacheOnly(ctx, scriptPath, scriptStageDir, requestedVersion)
	if err != nil {
		_ = os.RemoveAll(scriptStageDir)
		return err
	}
	targetVersion = normalizedSemver(targetVersion)
	if targetVersion == "" {
		_ = os.RemoveAll(scriptStageDir)
		return errors.New("install.ps1 cache phase returned an invalid payload version")
	}
	if requestedVersion != "" && requestedVersion != targetVersion {
		_ = os.RemoveAll(scriptStageDir)
		return fmt.Errorf("cached installer version %s does not match update version %s", targetVersion, requestedVersion)
	}

	if err := writeInstallScriptStageReady(scriptStageDir, targetVersion); err != nil {
		_ = os.RemoveAll(scriptStageDir)
		return err
	}
	UpdateDownloaded = true
...
func downloadInstallScript(ctx context.Context, scriptURL string, updateResp UpdateResponse) (string, bool, error) {
	...
	defer func() {
		if !downloadComplete {
			_ = os.RemoveAll(filepath.Dir(stageFilename))
		}
	}()
	...
	written, err := io.Copy(fp, io.LimitReader(resp.Body, maxInstallScriptSize+1))
	if err != nil {
		_ = fp.Close()
		_ = os.Remove(tmpFilename)
		return "", false, fmt.Errorf("write install.ps1 %s: %w", tmpFilename, err)
	}
	if written > maxInstallScriptSize {
		_ = fp.Close()
		_ = os.Remove(tmpFilename)
		return "", false, fmt.Errorf("install.ps1 exceeds maximum size of %d bytes", maxInstallScriptSize)
	}
...
func getStagedInstallScript() string {
	...
	for _, file := range files {
		stageDir := filepath.Dir(file)
		targetVersion, err := readInstallScriptStageVersion(stageDir)
		if err != nil {
			if errors.Is(err, os.ErrNotExist) {
				continue
			}
			slog.Warn("discarding staged update with invalid readiness metadata", "stage", stageDir, "error", err)
			removeInstallScriptStage(stageDir)
			continue
		}
		if stagedUpdateAlreadyInstalled(appversion.Version, targetVersion) {
			slog.Info("discarding staged update already satisfied by running app", "current", appversion.Version, "staged", targetVersion)
			removeInstallScriptStage(stageDir)
			continue
		}
...
func DoUpgrade(interactive bool) error {
	if script := getStagedInstallScript(); script != "" {
		return doInstallScriptUpgrade(script, interactive, exitAfterStartingUpgrade)
	}
	return fmt.Errorf("failed to lookup downloads")
}

func doInstallScriptUpgrade(script string, showProgress bool, exit func(int)) error {
	if script == "" {
		return fmt.Errorf("failed to lookup downloads")
	}
	...
	command := newInstallScriptInstallCommand(script, scriptStageDir, showProgress)
...
func runInstallScriptCacheOnlyCommand(ctx context.Context, scriptPath, scriptStageDir, version string) (string, error) {
	versionFile := filepath.Join(scriptStageDir, stagedCacheVersionFile)
	...
	if err != nil {
		return "", fmt.Errorf("read cached installer version: %w", err)
	}
	_ = os.Remove(versionFile)
...
func newInstallScriptCommand(scriptPath, workingDir, version, mode string) installScriptCommand {
	...
	if mode == installScriptModeCacheOnly {
		env = append(env, "OLLAMA_CACHE_VERSION_FILE="+filepath.Join(workingDir, stagedCacheVersionFile))
	}
...
func newInstallScriptInstallCommand(scriptPath, workingDir string, showProgress bool) installScriptCommand {
	command := newInstallScriptCommand(scriptPath, workingDir, "", installScriptModeInstallCached)
	if showProgress {
		command.Args = append(command.Args, "-ShowProgress")
	}
	return command
}
...
func stagedUpdateAlreadyInstalled(currentVersion, targetVersion string) bool {
	currentVersion = normalizedSemver(currentVersion)
	targetVersion = normalizedSemver(targetVersion)
	if currentVersion == "" || targetVersion == "" {
		return false
	}
	return semver.Compare("v"+currentVersion, "v"+targetVersion) >= 0
}

func readInstallScriptStageVersion(scriptStageDir string) (string, error) {
	readyFile := filepath.Join(scriptStageDir, stagedCacheReadyFile)
	data, err := os.ReadFile(readyFile)
	if err != nil {
		return "", err
	}
	targetVersion := normalizedSemver(string(data))
	if targetVersion == "" {
		return "", fmt.Errorf("invalid version in %s", readyFile)
	}
	return targetVersion, nil
}

func isInstallScriptStageReady(scriptStageDir, targetVersion string) bool {
	readyVersion, err := readInstallScriptStageVersion(scriptStageDir)
	if err != nil {
		return false
	}
	targetVersion = normalizedSemver(targetVersion)
	return targetVersion == "" || readyVersion == targetVersion
}

func writeInstallScriptStageReady(scriptStageDir, targetVersion string) error {
	targetVersion = normalizedSemver(targetVersion)
	if targetVersion == "" {
	...
	return os.WriteFile(filepath.Join(scriptStageDir, stagedCacheReadyFile), []byte(targetVersion+"\n"), 0o644)
}
...
func removeInstallScriptStage(scriptStageDir string) {
	if err := ensurePathInDir(UpdateStageDir, scriptStageDir); err != nil {
		slog.Warn("refusing to remove invalid update stage path", "stage", scriptStageDir, "error", err)
		return
	}
	if err := os.RemoveAll(scriptStageDir); err != nil {
		slog.Warn("failed to remove staged update", "stage", scriptStageDir, "error", err)
	}
}
