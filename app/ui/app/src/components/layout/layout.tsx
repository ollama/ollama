...
export function SidebarLayout({
  ...
  useEffect(() => {
    // Delay adding transition classes so they don't fire on initial load
    const timer = setTimeout(() => setIsMounted(true), 50);
    return () => clearTimeout(timer);
  }, []);

  return (
    <div className={`flex ${isMounted ? "transition-[width] duration-300" : ""} dark:bg-neutral-900`} >
      <div
        className={`absolute flex mx-2 py-2 z-20 items-center ${isMounted ? "transition-[left] duration-375" : ""} text-neutral-500 dark:text-neutral-400 ${settings.sidebarOpen ? (isWindows ? "left-2" : "left-[204px]") : isWindows ? "left-2" : "left-20"}`}
      >
        <button
      ...
      <div
        className={`flex flex-col ${isMounted ? "transition-[width] duration-300" : ""} max-h-screen ${settings.sidebarOpen ? "w-64" : "w-0"}`}
      >
        <div
      ...
      <main
        className={`flex flex-1 flex-col min-w-0 ${isMounted ? "transition-all duration-300" : ""}`}
      >
        <div
