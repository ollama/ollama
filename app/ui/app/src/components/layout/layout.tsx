...
export function SidebarLayout({
  ...
  // Track if this is the initial mount to prevent sidebar animation on load
  ...
  return (
    <div className={`flex ${isInitialMount ? "" : "transition-[width] duration-300"} dark:bg-neutral-900`}>
      <div
        className={`absolute flex mx-2 py-2 z-20 items-center ${isInitialMount ? "" : "transition-[left] duration-375"} text-neutral-500 dark:text-neutral-400 ${settings.sidebarOpen ? (isWindows ? "left-2" : "left-[204px]") : isWindows ? "left-2" : "left-20"}`}
      >
        <button
        ...
        <Link
          ...
          params={{ chatId: "new" }}
          title="New chat"
          className={`flex ml-1 items-center justify-center rounded-full ${isInitialMount ? "" : "transition-opacity duration-375"} h-9 w-9 hover:bg-neutral-100 dark:hover:bg-neutral-700 ${
            settings.sidebarOpen
      ...
      <div
        className={`flex flex-col ${isInitialMount ? "" : "transition-[width] duration-300"} max-h-screen ${settings.sidebarOpen ? "w-64" : "w-0"}`}
      >
        <div
      ...
      <main
        className={`flex flex-1 flex-col min-w-0 ${isInitialMount ? "" : "transition-all duration-300"}`}
      >
        <div
