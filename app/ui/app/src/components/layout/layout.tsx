...
export function SidebarLayout({
  ...
  // Enable transitions only after the first paint so the sidebar appears in
  // its initial state on load instead of animating open.
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden dark:bg-neutral-900">
      <div
        className={`absolute flex mx-2 py-2 z-20 items-center ${transitionsEnabled ? "transition-[left] duration-375" : ""} text-neutral-500 dark:text-neutral-400 ${sidebarOpen ? (isWindows ? "left-2" : "left-[140px]") : isWindows ? "left-2" : "left-20"}`}
      >
        <button
        ...
        {!title && (
          <Link
            ...
            params={{ chatId: "new" }}
            title="New chat"
            className={`flex ml-1 items-center justify-center rounded-full ${transitionsEnabled ? "transition-opacity duration-375" : ""} h-9 w-9 hover:bg-neutral-100 dark:hover:bg-neutral-700 ${
      ...
      <div
        className={`flex max-h-screen flex-col ${transitionsEnabled ? "transition-[width] duration-300" : ""} ${
          sidebarOpen
      ...
      <main
        className={`flex min-w-0 flex-1 flex-col ${transitionsEnabled ? "transition-all duration-300" : ""}`}
      >
        <div
        ...
        >
          {title && (
            <h1
              className={`${sidebarOpen ? "pl-6" : isWindows ? "pl-16" : "pl-36"} ${transitionsEnabled ? "transition-[padding-left] duration-300" : ""} font-rounded text-md font-medium dark:text-white`}
            >
              {title}
            </h1>
          )}
