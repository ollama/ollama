...
// migrateV19ToV20 adds the desktop update channel setting.
func (db *database) migrateV19ToV20() error {
	_, err := db.conn.Exec(`ALTER TABLE settings ADD COLUMN update_channel TEXT NOT NULL DEFAULT 'stable'`)
	if err != nil && !duplicateColumnError(err) {
		return fmt.Errorf("add update_channel column: %w", err)
	}
	if _, err = db.conn.Exec(`UPDATE settings SET schema_version = 20`); err != nil {
		return fmt.Errorf("update schema version: %w", err)
	}
	return nil
}
...
func (db *database) getSettings() (Settings, error) {
	...
	err := db.conn.QueryRow(`
		SELECT expose, survey, browser, models, agent, tools, working_dir, context_length, turbo_enabled, websearch_enabled, selected_model, sidebar_open, last_home_view, onboarding_version, think_enabled, think_level, auto_update_enabled, update_channel, claude_desktop_used, codex_desktop_used
		FROM settings
	`).Scan(&s.Expose, &s.Survey, &s.Browser, &s.Models, &s.Agent, &s.Tools, &s.WorkingDir, &s.ContextLength, &s.TurboEnabled, &s.WebSearchEnabled, &s.SelectedModel, &s.SidebarOpen, &s.LastHomeView, &s.OnboardingVersion, &s.ThinkEnabled, &s.ThinkLevel, &s.AutoUpdateEnabled, &s.UpdateChannel, &s.ClaudeDesktopUsed, &s.CodexDesktopUsed)
	if err != nil {
...
func (db *database) setSettings(s Settings) error {
	...
	_, err := db.conn.Exec(`
		UPDATE settings
		SET expose = ?, survey = ?, browser = ?, models = ?, agent = ?, tools = ?, working_dir = ?, context_length = ?, turbo_enabled = ?, websearch_enabled = ?, selected_model = ?, sidebar_open = ?, last_home_view = ?, onboarding_version = MAX(onboarding_version, ?), think_enabled = ?, think_level = ?, auto_update_enabled = ?, update_channel = ?, claude_desktop_used = ?
	`, s.Expose, s.Survey, s.Browser, s.Models, s.Agent, s.Tools, s.WorkingDir, s.ContextLength, s.TurboEnabled, s.WebSearchEnabled, s.SelectedModel, s.SidebarOpen, lastHomeView, s.OnboardingVersion, s.ThinkEnabled, s.ThinkLevel, s.AutoUpdateEnabled, NormalizeUpdateChannel(s.UpdateChannel), s.ClaudeDesktopUsed)
	if err != nil {
