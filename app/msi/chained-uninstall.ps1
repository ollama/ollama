...
$upgradeCodes = @(
  ...
  @{ UC="3E7A1B5C-9D2F-4A6E-B8C0-1F2D3E4A5B6C" }  # mlx_cuda_v13
...
if (-not $windowsDir) { exit 1 }
$msiExec = Join-Path $windowsDir "System32\msiexec.exe"
if (-not (Test-Path -LiteralPath $msiExec -PathType Leaf)) { exit 1 }
foreach ($pkg in $upgradeCodes) {
  try {
    $related = $installer.RelatedProducts("{$($pkg.UC)}")
    foreach ($pc in $related) {
      Start-Process -FilePath $msiExec -ArgumentList '/x',$pc,'/quiet','/norestart' -Wait -EA 0
    }
  } catch { }
}
