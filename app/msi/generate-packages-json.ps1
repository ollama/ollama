...
function Get-DependencyMsi {
    ...
    $versionPath = Join-Path $DistDir ("windows-{0}\lib\ollama\{1}\{2}" -f $Definition.Arch, $Definition.BackendPath, $Definition.VersionFile)
    if (Test-Path $versionPath) {
        $depsVersion = Get-Content -Path $versionPath -TotalCount 1 | Select-Object -First 1
        if ($depsVersion) {
            $depsVersion = $depsVersion.Trim()
        }
        if ($depsVersion) {
            ...
            if ($candidates[0].Name -ne $expectedDepsFile) {
                throw "Dependency MSI for $($Definition.Name) does not match $($Definition.VersionFile): found $($candidates[0].Name), expected $expectedDepsFile"
            }
            return $candidates[0]
        }
    }
