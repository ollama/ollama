...
typedef struct sycl_handle {
  ...
  // SYCL C wrapper function pointers
  int (*sycl_get_platform_count)();
  int (*sycl_get_device_count)();
  void (*sycl_get_device_ids)(int *id_list, int max_len);
  void (*sycl_get_device_name)(int device, char *name, size_t name_size);
  void (*sycl_get_device_vendor)(int device, char *vendor, size_t vendor_size);
  void (*sycl_get_device_memory)(int device, size_t *free, size_t *total);
  int (*sycl_is_gpu)(int device);
} sycl_handle_t;
