...
void musart_init(char *musart_lib_path, musart_init_resp_t *resp) {
  ...
  if (ret != MUSART_SUCCESS) {
    ...
    if (ret == MUSART_ERROR_INSUFFICIENT_DRIVER) {
      resp->err = strdup("your mthreads driver is too old or missing.  If you have a MUSA GPU please upgrade to run ollama");
      return;
    }
...
void musart_bootstrap(musart_handle_t h, int i, mem_info_t *resp) {
  ...
  LOG(h.verbose, "[%s] MUSA totalMem %" PRId64 "\n", resp->gpu_id, resp->total);
  LOG(h.verbose, "[%s] MUSA freeMem %" PRId64 "\n", resp->gpu_id, resp->free);
  LOG(h.verbose, "[%s] MUSA usedMem %" PRId64 "\n", resp->gpu_id, resp->used);
