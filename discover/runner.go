...
func GPUDevices(ctx context.Context, runners []FilteredRunnerDiscovery) []ml.DeviceInfo {
	...
	if !bootstrapped {
		...
		for dir := range libDirs {
			...
			newDevices := bootstrapDevices(ctx, []string{LibOllamaPath, dir}, nil)
			for _, dev := range newDevices {
				new := true
				for i := range devices {
					switch dev.Compare(devices[i]) {
					case ml.SameBackendDevice, ml.DuplicateDevice:
						new = false
					}
				}
				if new {
		...
		for i := range devices {
			if devices[i].Library == "HIP" {
				...
				go func(i int) {
					...
					if len(bootstrapDevices(ctx, devices[i].LibraryPath, extraEnvs)) == 0 {
						needsDelete[i] = true
					}
				}(i)
			}
		...
		// TODO - in the future if we add new backends that have overlapping devices, detect and remove here
