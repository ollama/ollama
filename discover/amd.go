...
func applyROCmLinuxSysfsDevice(device *ml.DeviceInfo, sysfsDevice rocmLinuxSysfsDevice) {
	...
	if sysfsDevice.known && sysfsDevice.integrated && sysfsDevice.gttKnown {
		correctROCmLinuxIntegratedMemory(device, sysfsDevice)
	}
}

func correctROCmLinuxIntegratedMemory(device *ml.DeviceInfo, sysfsDevice rocmLinuxSysfsDevice) {
...
func setROCmLinuxIntegratedFitTarget(device *ml.DeviceInfo, reportedFree, gttFree uint64) {
	...
	targetBytes := reportedFree - gttFree
	overreportedFreeMiB := (targetBytes + rocmLinuxIntegratedFitTargetUnit - 1) / rocmLinuxIntegratedFitTargetUnit
