...
func TestRefreshRunners(t *testing.T) {
	...
	type testCase struct {
		input  []discover.GpuInfo
		expect []string
	}
	testCases := map[string]*testCase{
		"cpu":     {input: []discover.GpuInfo{{Library: "cpu"}}, expect: []string{"cpu"}},
		"cpu_avx": {input: []discover.GpuInfo{{Library: "cpu", Variant: "avx"}}, expect: []string{"cpu_avx"}},
	}
	if runtime.GOOS == "darwin" && runtime.GOARCH == "arm64" {
		// macos arm doesn't append "cpu"
		testCases["foo no variant"] = &testCase{input: []discover.GpuInfo{{Library: "foo"}}, expect: []string{"foo_v11", "foo_v12"}}
		testCases["foo v11"] = &testCase{input: []discover.GpuInfo{{Library: "foo", Variant: "v11"}}, expect: []string{"foo_v11", "foo_v12"}}
		testCases["foo v12"] = &testCase{input: []discover.GpuInfo{{Library: "foo", Variant: "v12"}}, expect: []string{"foo_v12", "foo_v11"}}
		testCases["foo v11 and v12"] = &testCase{input: []discover.GpuInfo{{Library: "foo", Variant: "v11"}, {Library: "foo", Variant: "v12"}}, expect: []string{"foo_v11", "foo_v12"}}
	} else {
		// All other platforms append "cpu" as a last ditch runner
		testCases["foo no variant"] = &testCase{input: []discover.GpuInfo{{Library: "foo"}}, expect: []string{"foo_v11", "foo_v12", "cpu"}}
		testCases["foo v11"] = &testCase{input: []discover.GpuInfo{{Library: "foo", Variant: "v11"}}, expect: []string{"foo_v11", "foo_v12", "cpu"}}
		testCases["foo v12"] = &testCase{input: []discover.GpuInfo{{Library: "foo", Variant: "v12"}}, expect: []string{"foo_v12", "foo_v11", "cpu"}}
		testCases["foo v11 and v12"] = &testCase{input: []discover.GpuInfo{{Library: "foo", Variant: "v11"}, {Library: "foo", Variant: "v12"}}, expect: []string{"foo_v11", "foo_v12", "cpu"}}
	}
