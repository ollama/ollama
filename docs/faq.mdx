...
- `tq3k` - TurboQuant 3-bit K-only quantization, uses approximately 2/5 the memory of `f16` with a negligible quality impact, and unlike `q4_0`/`q8_0` does not require Flash Attention to be enabled.
- `tq2k` - TurboQuant 2-bit K-only quantization, uses approximately 1/3 the memory of `f16` with a small quality impact, and unlike `q4_0`/`q8_0` does not require Flash Attention to be enabled.
