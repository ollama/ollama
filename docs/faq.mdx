...
- `tq4v` - TurboQuant 4-bit V-only quantization, uses approximately 2/3 the memory of `f16` with near-`f16` quality. The `tq*v` presets compress only the V cache (K stays `f16`); because V tolerates quantization more gracefully than K, they trade the same memory for K-only at equal or better quality, and like `tq*k` they do not require Flash Attention.
- `tq3v` - TurboQuant 3-bit V-only quantization, uses approximately 3/5 the memory of `f16` with negligible quality impact.
- `tq2v` - TurboQuant 2-bit V-only quantization, uses approximately 1/2 the memory of `f16` with a small quality impact.
