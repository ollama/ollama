...
<Note>
  On Linux using the standard installer, the `ollama` user needs read and write access to the specified directory. To assign the directory to the `ollama` user run `sudo chown -R ollama:ollama <directory>` . In certain cases (like separate drives), you may also need to do `sudo chmod -R 755 <drive-mounting-point>` (e.g. `/media/username` )
</Note>
