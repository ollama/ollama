...
## How can I auto-optimize Ollama for my hardware?

Ollama includes an **autotune** feature that automatically detects your hardware (CPU, GPU, VRAM, RAM) and applies optimal settings. Instead of manually configuring Flash Attention, KV cache type, context length, and other parameters, you can set a single performance profile.
...
Set the `OLLAMA_PERFORMANCE_PROFILE` environment variable before starting the server:
...
You can also change the profile at runtime using the CLI:

```shell
ollama autotune speed
```

Check current autotune status:

```shell
ollama autotune status
```
...
<Note>
  Autotune only sets values for environment variables that are not already explicitly set.
  Your manual overrides always take priority.
</Note>
