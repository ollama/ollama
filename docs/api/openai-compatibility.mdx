...
Ollama supports passing native parameters through the `options` field while using OpenAI-compatible endpoints.

- **`think`** (boolean): Enable thinking/reasoning mode for models that support it.
- **`keep_alive`** (string or number): Keep the model loaded in memory after a request (for example `"30m"` or seconds as a number).
