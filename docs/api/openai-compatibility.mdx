...
For both `/v1/chat/completions` and `/v1/responses`, reasoning effort only supports the following values:
...
Values such as `"minimal"` and `"xhigh"` are not supported and will result in an error.
