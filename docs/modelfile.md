...
|image_data      | Only supported for multimodal models (e.g., LLaVAAn). Array of objects to hold base64-encoded image `data` and its `id`s to be reference in `prompt`. You can determine the place of the image in the prompt as follows: `[img-12]Describe the image in detail.` where `[img-12]` is the reference to the image with an id of 12. | [{data: string, id: int | string}] | image_data: [{"data": "<BASE64_STRING>", "id": 12}]
