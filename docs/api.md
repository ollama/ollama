...
Convert text to token IDs and back using a model's vocabulary. These endpoints are optimized for vocab-only operations to avoid full model loading when possible.
...
- `model`: (required) the [model name](#model-names)
- `content`: (required) the text to convert to token IDs
- `options`: additional model parameters (optional)

#### Detokenize Request

- `model`: (required) the [model name](#model-names)  
- `tokens`: (required) array of token IDs to convert back to text
- `options`: additional model parameters (optional)

### Notes

- **Model-aligned**: Token IDs are specific to each model's vocabulary
- **Text-only**: Multimodal support reserved for future versions
- **No keep_alive required**: Vocab-only path aims to avoid full model loads/VRAM allocation
- **Timing fields**: For observability and performance monitoring
...
| Model            | Mode         | Load Path          | P50 latency |
|------------------|--------------|--------------------|-------------|
| mistral:latest   | cold         | fallback scheduler | ~3.42s      |
| mistral:latest   | warm (cache) | vocab-only (cache) | ~10–18ms    |
| tinyllama:latest | cold         | fallback scheduler | ~0.89s      |
| tinyllama:latest | warm (cache) | vocab-only (cache) | ~10–20ms    |

> **Note**: When vocab-only hook is wired, cold times should drop significantly; fallback preserves correctness meanwhile.
