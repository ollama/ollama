...
Create a model from GGUF files. The `files` parameter should contain the file name and SHA256 digest of each GGUF file. Use [/api/blobs/:digest](#push-a-blob) to push each file before calling this API.
For a model stored in multiple split GGUF files, include all split GGUF files in the `files` parameter with their file names and SHA256 digests. It is recommended to provide files in split number order, though Ollama sorts them by split index itself.
