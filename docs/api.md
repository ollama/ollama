...
When `model_format` is set to `"safetensors"`, the server assembles a native safetensors manifest directly from pre-uploaded tensor blobs. Each tensor is stored as an individual content-addressed blob, and the `files` map uses tensor names (e.g. `"model.embed_tokens.weight"`) as keys. JSON config files use their file names as keys.
