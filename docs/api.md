...
- `capabilities`: (optional) list of model capabilities (e.g. `"completion"`, `"image"`, `"thinking"`). Used with `model_format="safetensors"`.
...
> **Note:** When `quantize` is used with `model_format="safetensors"`, the client uploads unquantized tensor blobs and the server quantizes eligible tensors (e.g. attention/MLP weights) during manifest assembly. Tensors that are not eligible for quantization (e.g. embeddings, norms, biases) are stored as-is. Server-side quantization requires MLX support on the server.
