...
Safetensors imports use MLX and can be quantized during import. Prepare GGUF files with community tools before importing them; `ollama create` does not convert or quantize GGUF files.

If you are creating a model from a safetensors directory or from a GGUF file, you must [push a blob](#push-a-blob) for each file and then use its file name and SHA256 digest in the `files` field.
...
- `quantize` (optional): quantize safetensors weights during import
...
Quantize safetensors weights during import.
...
```json
{"status":"importing fred-nvfp4 (2 tensors, quantizing to nvfp4)"}
{"status":"writing manifest for fred-nvfp4"}
{"status":"success"}
```

#### Create a model from GGUF

Create a model from a GGUF file. The `files` parameter should contain the file name and SHA256 digest of the GGUF file. Use [/api/blobs/:digest](#push-a-blob) to push the file before calling this API.
