...
```
ollama embed embeddinggemma "Hello world"
```

Output is space-separated floats by default. Use `--format json` for structured output with metadata.

```
echo "Hello world" | ollama embed nomic-embed-text --format json
```
