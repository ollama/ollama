...
The environment variable `OLLAMA_DRAFT_GPU_DEVS` limits loading of the draft model to certain GPU devices (e.g., `OLLAMA_DRAFT_GPU_DEVS="CUDA0,CUDA3"). By default all GPU devices available for the target model are also used for the draft model. <b>Note: not functional yet!</b>
