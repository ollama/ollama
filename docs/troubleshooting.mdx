...
Set which log level to use with the `OLLAMA_DEBUG` environment variable:
```
OLLAMA_DEBUG= or OLLAMA_DEBUG=0 (default): basic information
OLLAMA_DEBUG=1 or OLLAMA_DEBUG=true: set DEBUG level, may include information about prompts and responses
OLLAMA_DEBUG=2: TRACE level, includes prompts and responses
```
