...
ROCm userspace libraries which require a compatible AMD kernel driver. With an
older driver, GPU initialization can hang during device discovery and
eventually time out, causing Ollama to fall back to CPU.

To resolve this, upgrade to a current driver using the `amdgpu-install` utility
from [AMD's ROCm documentation](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/).
