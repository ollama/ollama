...
For experimental forks or local builds, you can also set `OLLAMA_AMD_UNSAFE=1`
to skip the ROCm deep-init filtering step and let Ollama attempt to use an
unsupported AMD device. This is not recommended for normal builds because the
runner can still crash during inference.
