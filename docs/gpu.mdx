...
To add the experimental TheRock ROCm package to the default Windows install:

```powershell
$Ollama = "$env:LOCALAPPDATA\Programs\Ollama"; $Zip = "$env:TEMP\ollama-windows-amd64-rocm-nightly.zip"
Invoke-WebRequest https://ollama.com/download/ollama-windows-amd64-rocm-nightly.zip -OutFile $Zip; Expand-Archive -Force $Zip $Ollama
```

TheRock may add experimental support for newer ROCm targets, including some Ryzen AI and Radeon RX 9000-class GPUs, but support depends on the installed AMD driver stack.
