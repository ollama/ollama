...
If you want Copilot CLI to talk to a remote Ollama instance or a remote OpenAI-compatible provider, set the provider URL explicitly:

```shell
export COPILOT_PROVIDER_BASE_URL=https://remote.example.com/v1
...
If `COPILOT_PROVIDER_BASE_URL` is not set, `ollama launch copilot` will fall back to `OLLAMA_HOST` and then the local Ollama host.

For example, this points Copilot CLI at a remote Ollama server:

```shell
export OLLAMA_HOST=https://remote.example.com
...
jobs:
  copilot-headless:
    ...
    steps:
      ...
      - name: Run Copilot CLI headlessly
        env:
          COPILOT_PROVIDER_BASE_URL: https://remote.example.com/v1
          COPILOT_PROVIDER_API_KEY: ""
          COPILOT_PROVIDER_WIRE_API: responses
          OLLAMA_HOST: https://remote.example.com
        run: |
          ollama launch copilot --model qwen3.5 --yes -- -p "Summarize the repository and suggest next steps."
```
