...
If your Codex configuration includes the `model_reasoning_effort` setting, make sure it uses a value supported by Ollama.
...
Values such as `minimal` and `xhigh` are not supported and will result in an error when sending requests through Ollama.

For example, replace an unsupported value such as:
...
```text
invalid reasoning value: "xhigh"
```

your `model_reasoning_effort` setting uses a value that Ollama does not support. Update it to one of the supported values:
...
your version of Ollama does not support the Codex App integration. Upgrade to a version of Ollama that supports Codex App, then run:
