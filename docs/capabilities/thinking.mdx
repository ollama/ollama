...
## Set a default in a Modelfile

Use `PARAMETER think` to bake a default thinking mode into a model. The setting is inherited by derived models via `FROM` and included when saving with `/save`.
...
Requests that set `think` explicitly override the Modelfile default.
