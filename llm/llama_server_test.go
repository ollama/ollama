...
func TestAppendMMProjArgs(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "integrated rocm gpu with large memory keeps projector offload",
			projectors:   []string{"model.gguf"},
			opts:         defaultOpts,
		...
		{
			name:         "integrated rocm gpu with limited memory disables projector offload",
			projectors:   []string{"model.gguf"},
			opts:         defaultOpts,
			gpus:         []ml.DeviceInfo{{DeviceID: ml.DeviceID{Library: "ROCm"}, Integrated: true, FreeMemory: 1500 << 20, TotalMemory: 8 << 30}},
			mmprojMemory: 933 << 20,
			modelLayers:  81,
			want:         []string{"base", "--mmproj", "model.gguf", "--no-mmproj-offload"},
		},
