...
COMMON_CMAKE_DEFS="-DCMAKE_POSITION_INDEPENDENT_CODE=on -DLLAMA_NATIVE=off -DLLAMA_AVX=on -DLLAMA_AVX2=off -DLLAMA_AVX512=off -DLLAMA_FMA=off -DLLAMA_F16C=off -DGGML_OPENMP=off"
...
if [ -z "${OLLAMA_SKIP_CPU_GENERATE}" ]; then
    ...
    if [ -n "${OLLAMA_CUSTOM_CPU_DEFS}" ]; then
        ...
        COMMON_CPU_DEFS="-DCMAKE_POSITION_INDEPENDENT_CODE=on -DLLAMA_NATIVE=off -DGGML_OPENMP=off"
