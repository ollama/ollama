...
if [ -d "${ONEAPI_ROOT}" ]; then
    ...
    CC=icx
    CMAKE_DEFS="${COMMON_CMAKE_DEFS} ${CMAKE_DEFS} -DCMAKE_C_COMPILER=icx -DCMAKE_CXX_COMPILER=icpx -DLLAMA_SYCL=ON"
    ...
    CGO_CFLAGS="" # icx compiles with -O0 if we pass -g, so we must remove it
    build
