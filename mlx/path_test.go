...
func TestDependencyDirRejectsNonCUDAVariant(t *testing.T) {
	root := t.TempDir()
	for _, dir := range []string{"mlx_metal_v3", "metal_v3"} {
		if err := os.Mkdir(filepath.Join(root, dir), 0o755); err != nil {
			t.Fatal(err)
		}
	}

	if depDir, ok := dependencyDir(filepath.Join(root, "mlx_metal_v3")); ok {
		t.Fatalf("dependencyDir() = %q, true; want non-CUDA variant rejected", depDir)
	}
}
