...
func modulateLayerNorm(x *mlx.Array, shift, scale *mlx.Array) *mlx.Array {
	// LayerNorm without learnable params: (x - mean) / sqrt(var + eps)
	lastAxis := len(x.Shape()) - 1
	mean := mlx.Mean(x, lastAxis, true)
	xCentered := mlx.Sub(x, mean)
	variance := mlx.Mean(mlx.Square(xCentered), lastAxis, true)
	x = mlx.Div(xCentered, mlx.Sqrt(mlx.AddScalar(variance, 1e-6)))
...
func applyQKNorm(x *mlx.Array, scale *mlx.Array) *mlx.Array {
	...
	variance := mlx.Mean(mlx.Square(x), len(x.Shape())-1, true)
	x = mlx.Div(x, mlx.Sqrt(mlx.AddScalar(variance, 1e-6)))
	// Apply learned scale
	return mlx.Mul(x, scale)
}
