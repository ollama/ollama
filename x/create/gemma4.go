...
func (t gemma4ImportTransform) quantizationType(name string, shape []int32, quantize string) string {
	...
	// lm_head projects to the full 262K vocab — quantization errors here directly
	// affect token selection. Use higher precision when the base quant is 4-bit.
	...
	//
	// For int4: promote → int8 (same affine mode, GatherQMM compatible).
	// For mxfp4: promote → mxfp8 (same mxfp family, group_size=32, fp8_e8m0
	//   scales, MLX QuantizationMode::Mxfp8 is supported).
	// For nvfp4: no promotion available. MLX has no Nvfp8 quantize mode —
	//   the quantize op only supports {Affine, Nvfp4, Mxfp4, Mxfp8}.
	// Each quant family uses different scale formats and must never be mixed.
	if t.numLayers > 0 {
		layerIdx := -1
		...
		// "mxfp8" = mxfp4 base → mxfp8 (mxfp family)
		// ""      = no promotion (nvfp4/int8/mxfp8, already 8-bit or no FP8 support)
		promote := ""
		switch quantNorm {
		case "int4":
			promote = "int8"
		...
		// Only apply mixed-precision logic when we have a promotion target.
...
// toLMHeadName converts an embed_tokens weight name to an lm_head name.
// e.g. "model.language_model.embed_tokens.weight" -> "model.language_model.lm_head.weight"
func toLMHeadName(name string) string {
	return strings.Replace(name, "embed_tokens.weight", "lm_head.weight", 1)
}
...
func (t gemma4ImportTransform) transformTensor(td *safetensors.TensorData) ([]*safetensors.TensorData, error) {
	...
	// When the model has tied embeddings (no separate lm_head), create
	// a copy of embed_tokens as lm_head so it can be quantized independently.
	// The embedding stays BF16 for lookup quality; the LM head gets quantized
	// to int8 for faster generation (it's the single largest per-token matmul).
	if isEmbedTokensWeight(td.Name) {
		lmHead := td.WithName(toLMHeadName(td.Name))
		return []*safetensors.TensorData{td, lmHead}, nil
	}
