...
func FuzzOpenForExtraction(f *testing.F) {
	valid, err := io.ReadAll(BuildPackedSafetensorsReaderWithMetadata([]*TensorData{
		NewTensorDataFromBytes("weight", "F32", []int32{2}, make([]byte, 8)),
	}, map[string]string{"format": "pt"}))
	if err != nil {
		f.Fatal(err)
	}
	oversized := make([]byte, 8)
	binary.LittleEndian.PutUint64(oversized, uint64(maxSafetensorsHeaderSize)+1)
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		for _, name := range ext.ListTensors() {
			...
			if err != nil {
				t.Fatalf("GetTensor(%q) after successful open: %v", name, err)
			}
			raw, err := io.ReadAll(tensor.Reader())
			if err != nil {
				t.Fatalf("read tensor %q: %v", name, err)
			}
			if int64(len(raw)) != tensor.Size {
				t.Fatalf("tensor %q size = %d, want %d", name, len(raw), tensor.Size)
			}

			roundTrip, err := ExtractRawFromSafetensors(tensor.SafetensorsReader())
			if err != nil {
				t.Fatalf("round-trip tensor %q: %v", name, err)
			}
			if !bytes.Equal(roundTrip, raw) {
				t.Fatalf("round-trip tensor %q data mismatch", name)
			}
		}
	})
}
