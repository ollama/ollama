...
def load_model_auto(model_path, dtype, attn_implementation=None):
...
def load_model_class(model_path, dtype, class_name, attn_implementation=None):
    ...
    if cls is None:
        raise RuntimeError(
            ...
            f"If this is a model that requires a custom transformers source, "
            f"set --transformers-path or TRANSFORMERS_PATH to point at it."
        )
    kwargs = {"torch_dtype": dtype}
    if attn_implementation:
        kwargs["attn_implementation"] = attn_implementation
...
def main():
    ...
    if args.model_class:
        model = load_model_class(args.model, dtype, args.model_class, args.attn_implementation)
    else:
        model = load_model_auto(args.model, dtype, args.attn_implementation)
