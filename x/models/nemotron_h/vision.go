...
const (
	...
	nemotronVisionDefaultIntermediateSize = int32(5120)
	nemotronVisionDefaultProjectorHidden  = int32(20480)
...
var (
	nemotronVisionDefaultMean = [3]float32{0.48145466, 0.4578275, 0.40821073}
	nemotronVisionDefaultStd  = [3]float32{0.26862954, 0.26130258, 0.27577711}
)
...
func parseVisionConfig(configData, preprocessorData []byte) (*VisionConfig, error) {
	var env struct {
		VisionConfig *struct {
			...
			Intermediate    int32  `json:"intermediate_size"`
			Args            struct {
				MinNumPatches int    `json:"min_num_patches"`
				MaxNumPatches int    `json:"max_num_patches"`
	...
	cfg := &VisionConfig{
		...
		IntermediateSize:    firstPositiveInt32(env.VisionConfig.Intermediate, nemotronVisionDefaultIntermediateSize),
		...
		ProjectorHiddenSize: firstPositiveInt32(env.ProjectorHiddenSize, nemotronVisionDefaultProjectorHidden),
		...
		Mean:                nemotronVisionDefaultMean,
		Std:                 nemotronVisionDefaultStd,
	...
	setTriplet(cfg.Mean[:], env.NormMean)
	setTriplet(cfg.Std[:], env.NormStd)
...
func (cfg *VisionConfig) applyPreprocessorConfig(data []byte) error {
	...
	setTriplet(cfg.Mean[:], pre.NormMean)
	setTriplet(cfg.Std[:], pre.NormStd)
	return nil
}
...
func setTriplet(dst []float32, src []float32) {
	if len(src) != 3 {
		return
	}
	copy(dst, src)
...
func (p *VisionProjector) Forward(x *mlx.Array) *mlx.Array {
	x = p.Norm.Forward(x, p.Cfg.ProjectorNormEps)
	x = p.FC1.Forward(x)
	x = mlx.ReLUSquared(x)
