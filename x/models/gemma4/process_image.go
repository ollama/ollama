...
// visionTargetSize ports the reference aspect-preserving resize: target
// dimensions are floored to multiples of patchSize*poolingKernel under a
// total patch budget; a dimension that floors to zero is clamped to one
// side multiple with the other capped at the widest the budget allows.
...
// rescale to [0,1], and patchify to the reference layout — one row per
// patch, (pixel row, pixel column, RGB) within it. The [-1,1] normalization
// stays in the patch embedder, so pixels here match the reference
// pixel_values.
func (m *Model) preprocessImage(data []byte) (pixels []float32, positions []int32, geom ImageGeometry, err error) {
	...
	bounds := img.Bounds()
	targetH, targetW, err := visionTargetSize(int32(bounds.Dy()), int32(bounds.Dx()), patch, pool, m.softTokenBudget()*pool*pool)
	if err != nil {
