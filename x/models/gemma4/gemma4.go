...
// buildCausalMask creates a [1, 1, Q, K] additive causal mask.
// Positions where kv > q_offset + q get -inf; others get 0.
func buildCausalMask(Q, K int32) *mlx.Array {
...
type MoEBlock struct {
	...
	DownQuantMode                             string // down mode (may differ for mixed nvfp4/nvfp8)
}
		...
		if m.EnableMoeBlock {
			...
			if gateUpW != nil {
				...
				if gateUpScales != nil && downScales != nil {
					// Quantized: split fused gate_up along axis 1 for weight, scales, biases.
					dims := switchGateUp.Dims()
					mid := int32(dims[1] / 2)
					...
					moe.GateWeightQ = sliceAxis1(switchGateUp, 0, mid)
					moe.UpWeightQ = sliceAxis1(switchGateUp, mid, int32(dims[1]))
					moe.DownWeightQ = switchDown

					sDims := gateUpScales.Dims()
					sMid := int32(sDims[1] / 2)
					moe.GateScales = sliceAxis1(gateUpScales, 0, sMid)
					moe.UpScales = sliceAxis1(gateUpScales, sMid, int32(sDims[1]))
					moe.DownScales = downScales

					if gateUpBiases != nil {
						bDims := gateUpBiases.Dims()
						bMid := int32(bDims[1] / 2)
						moe.GateBiases = sliceAxis1(gateUpBiases, 0, bMid)
						moe.UpBiases = sliceAxis1(gateUpBiases, bMid, int32(bDims[1]))
					}
					if downBiases != nil {
					...
					moe.GateGroupSize = groupSize
					moe.GateBits = bits
					moe.UpGroupSize = groupSize
					moe.UpBits = bits
					moe.QuantMode = mode
					...
					// Unquantized switch_mlp: split and transpose for GatherMM.
					dims := switchGateUp.Dims()
					mid := int32(dims[1] / 2)
					moe.GateWeight = transposeForGatherMM(sliceAxis1(switchGateUp, 0, mid))
					moe.UpWeight = transposeForGatherMM(sliceAxis1(switchGateUp, mid, int32(dims[1])))
...
func (m *Model) EmbedAndScale(tokens *mlx.Array) *mlx.Array {
	h := m.EmbedTokens.Forward(tokens)
	return mlx.MulScalar(h, float32(math.Sqrt(float64(m.HiddenSize))))
}
...
func (m *Model) Unembed(x *mlx.Array) *mlx.Array {
	...
	if m.FinalLogitSoftcapping > 0 {
		logits = mlx.MulScalar(logits, 1.0/m.FinalLogitSoftcapping)
		logits = logits.Tanh()
...
func (m *Model) computePLEInputs(tokens, h *mlx.Array) *mlx.Array {
	dims := tokens.Dims()
	B, L := int32(dims[0]), int32(dims[1])
	pleScale := float32(math.Sqrt(float64(m.HiddenSizePerLayer)))
	projScale := float32(1.0 / math.Sqrt(float64(m.HiddenSize)))
	...
	combined = mlx.MulScalar(combined, float32(math.Pow(2.0, -0.5)))
...
func (a *Attention) Forward(x *mlx.Array, c cache.Cache, B, L int32, isSliding bool, cfg *TextConfig, donorEntry *sharedKVEntry) *mlx.Array {
	...
	var out *mlx.Array
	if headDim > 128 && L > 1 {
		// Manual attention for prefill (L > 1) with head_dim > 128.
		// cuDNN SDPA requires head_dim <= 128, and the MLX CUDA SDPA vector
		// kernel only handles L < 4 (generation). For prefill, we fall back
		// to explicit matmul+softmax+matmul.
		// Generation (L == 1) uses SDPA's vector kernel which supports
		// head_dim=256 with f32 accumulation.
		// On Metal, SDPA handles all head dims and sequence lengths natively.
	...
	// Force contiguous layout before OProj to ensure consistent matmul behavior
	// across Metal and CUDA backends. The Transpose above creates a strided view
	// that CUDA matmul handles differently from Metal.
...
func (r *Router) Forward(x *mlx.Array, cfg *TextConfig) (*mlx.Array, *mlx.Array) {
	...
	normed = mlx.MulScalar(normed, float32(1.0/math.Sqrt(float64(cfg.HiddenSize))))
...
func (m *MoEBlock) Forward(x *mlx.Array, scores, inds *mlx.Array, cfg *TextConfig) *mlx.Array {
	...
	// Expert computation: gate_proj and up_proj followed by GELU and down_proj.
	var gate, up, hidden, down *mlx.Array
	if m.UseQuantized {
		// transpose=true: weight is [experts, out, in_packed], GatherQMM transposes
		// to [experts, in, out] internally before matmul (matching qwen3.5 pattern).
		...
		// Unquantized GatherMM: weights already transposed at load time to
		// [experts, in, out] for GatherMM (which computes a @ b[group]).
