...
func TestFastMoEGatherMMBlockMappedMatchesGatherMM(t *testing.T) {
	...
	withMLXThread(t, func() {
		...
		for _, tc := range []struct {
			...
			if !runMoEGatherMMBlockMappedTestCase(tc.name+"_input_slots_1", tc.tokens, tc.topK, tc.inDim, tc.outDim, 1, dtype, w, idx, expertMap, failf) {
				return
			}
			if !runMoEGatherMMBlockMappedTestCase(tc.name+"_input_slots_topk", tc.tokens, tc.topK, tc.inDim, tc.outDim, tc.topK, dtype, w, idx, expertMap, failf) {
				return
			}
		}
	})
...
func runMoEGatherMMBlockMappedTestCase(name string, tokens, topK, inDim, outDim, inputSlots int, dtype DType, w, idx *Array, expertMap *MoEExpertMap, failf func(string, ...any)) bool {
	...
	Pin(x, w, idx, expertMap.counts, expertMap.blockCount, expertMap.blockExperts, expertMap.blockOffsets, expertMap.blockIDs)
	defer Unpin(x, w, idx, expertMap.counts, expertMap.blockCount, expertMap.blockExperts, expertMap.blockOffsets, expertMap.blockIDs)
	...
	var wantRelu *Array
	if inputSlots == 1 {
		want = GatherMM(Reshape(x, tokens32, 1, 1, inDim32), w, nil, idx, false)
		wantRelu = GatherMM(Reshape(xRelu, tokens32, 1, 1, inDim32), w, nil, idx, false)
	} else {
		want = GatherMM(Reshape(x, tokens32, topK32, 1, inDim32), w, nil, idx, false)
		wantRelu = GatherMM(Reshape(xRelu, tokens32, topK32, 1, inDim32), w, nil, idx, false)
	}
