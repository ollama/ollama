...
func TestMamba2ScanMatchesReference(t *testing.T) {
	withMLXThread(t, func() {
		...
		if len(interior) != 0 {
			t.Fatalf("interior states = %d, want 0 without captureAll", len(interior))
		}
...
func TestMamba2ScanCaptureAllMatchesPerTokenReference(t *testing.T) {
	withMLXThread(t, func() {
		...
		if len(gotInterior) != T-1 {
			t.Fatalf("interior states = %d, want %d", len(gotInterior), T-1)
		}
		...
		for ti := range int32(T) {
			...
			assertArrayClose(t, fmt.Sprintf("token %d y", ti), sliceTimeRange(gotY, ti, ti+1), y, 1e-5)
			if int(ti) < T-1 {
				assertArrayClose(t, fmt.Sprintf("token %d interior state", ti), gotInterior[ti], state, 1e-5)
			}
		}
		assertArrayClose(t, "captureAll end state", gotEnd, state, 1e-5)
	})
}
...
func TestMamba2ScanUnsupportedShapeMatchesGraph(t *testing.T) {
	withMLXThread(t, func() {
		...
		if _, ok := resolveMamba2ScanDims(in.hidden, in.bState, in.cState, in.dt, in.state, in.a, in.d, in.dtBias); ok {
			t.Fatal("resolveMamba2ScanDims ok=true for unsupported S=31 shape")
		}
		gotY, gotState, _ := Mamba2Scan(in.hidden, in.bState, in.cState, in.dt, in.state, in.a, in.d, in.dtBias, nil, false)
		wantY, wantState := mamba2ScanReference(in.hidden, in.bState, in.cState, in.dt, in.state, in.a, in.d, in.dtBias)
		assertArrayClose(t, "unsupported-shape y", gotY, wantY, 1e-5)
		assertArrayClose(t, "unsupported-shape state", gotState, wantState, 1e-5)
	})
}
...
func TestMamba2ScanPaddedRowIsIdentity(t *testing.T) {
	withMLXThread(t, func() {
		...
		for row, realLen := range []int32{L, 1} {
			...
			assertArrayClose(t, fmt.Sprintf("row %d y", row), sliceRowTime(gotY, r, realLen), wantY, 1e-5)
			assertArrayClose(t, fmt.Sprintf("row %d state", row), sliceRow(gotState, r), wantState, 1e-5)
		}
		...
		for i, v := range pad.Floats() {
			if v != 0 {
				t.Fatalf("padded output[%d] = %v, want 0", i, v)
			}
		}
	})
}
