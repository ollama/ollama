...
for (int loop_k = 0; loop_k < K; loop_k += NK) {
  ...
  const size_t w_base = (size_t(expert) * K + size_t(k0)) * N + size_t(col);
  ...
  MOE_MAPPED_FOR_UNROLL (short i = 0; i < 16; i++) {
    ...
    *(sa + NK * (8 * sy + ly) + 8 * sx + lx) = bfloat(static_cast<float>(w[w_base + size_t(i) * N]));
  }
...
#else
// Dense BF16 mapped MM is enabled from Go only when Metal tensor ops are
// available. Older Metal has no BF16 simdgroup matrix path; FP16 simdgroup math
// corrupts Laguna BF16 prompt quality, and scalar float did not improve over
// generic GatherMM in testing.
return;
#endif
`
...
func addMoEMappedTemplateArgs(cfg C.mlx_fast_metal_kernel_config, tokens, topK, inputSlots, experts, N, K int, outDType DType, applyReluSquared bool) bool {
	reluSquaredValue := 0
	if applyReluSquared {
		reluSquaredValue = 1
	}
...
func fastMoEExpertMap(indices *Array, experts int) (counts, ids *Array, ok bool) {
	...
	if C.mlx_fast_metal_kernel_config_set_verbose(cfg, C.bool(false)) != 0 {
		return nil, nil, false
	}
...
func moeGatherMMSupportedDType(dtype DType) bool {
	return dtype == DTypeBFloat16 && metalTensorOpsAvailable()
}

func moeGatherMMBlockMappedValidate(x, w, counts, blockCount, blockExperts, blockOffsets, blockIDs *Array, topK int) (tokens, inputSlots, experts, N, K, maxBlocks int, ok bool) {
...
func applyMoEGatherMappedKernel(kernel C.mlx_fast_metal_kernel, name string, outDType DType, inputs []C.mlx_array, tokens, topK, inputSlots, experts, N, K, gridX, gridY, gridZ int, applyReluSquared bool) (y *Array, ok bool) {
	cfg := C.mlx_fast_metal_kernel_config_new()
	defer C.mlx_fast_metal_kernel_config_free(cfg)
	if C.mlx_fast_metal_kernel_config_set_verbose(cfg, C.bool(false)) != 0 {
		return nil, false
	}
	if !addMoEMappedTemplateArgs(cfg, tokens, topK, inputSlots, experts, N, K, outDType, applyReluSquared) {
		return nil, false
	}
...
// dispatch dense expert weights with the given dtype. Dense BF16 uses Metal
// tensor ops so the fallback does not route BF16 through FP16 simdgroup math.
...
func fastMoEGatherMMBlockMapped(x, w *Array, expertMap *MoEExpertMap, topK int, applyReluSquared bool) (y *Array, ok bool) {
	...
	tokens, inputSlots, experts, N, K, maxBlocks, ok := moeGatherMMBlockMappedValidate(x, w, expertMap.counts, expertMap.blockCount, expertMap.blockExperts, expertMap.blockOffsets, expertMap.blockIDs, topK)
	if !ok {
		return nil, false
	}
	...
	out, ok := applyMoEGatherMappedKernel(moeGatherMMBlockMappedMetalKernel, "MOE_GATHER_MM_BLOCK_MAPPED", x.DType(), inputs, tokens, topK, inputSlots, experts, N, K, gridX, gridY, 1, applyReluSquared)
	if !ok {
		return nil, false
	}
	return out, true
...
func fastNVFP4MoEGatherQMMMapped(x, w, scales, counts, ids *Array, topK int, applyReluSquared bool) (y *Array, ok bool) {
	...
	return applyMoEGatherMappedKernel(nvfp4MoEMappedMetalKernel, "MOE_GATHER_QMM_MAPPED", x.DType(), inputs, tokens, topK, inputSlots, experts, N, K, gridX, gridY, experts, applyReluSquared)
}
...
func fastNVFP4MoEGatherQMMBlockMapped(x, w, scales, counts, blockCount, blockExperts, blockOffsets, blockIDs *Array, topK int, applyReluSquared bool) (y *Array, ok bool) {
	...
	return applyMoEGatherMappedKernel(nvfp4MoEBlockMappedMetalKernel, "MOE_GATHER_QMM_BLOCK_MAPPED", x.DType(), inputs, tokens, topK, inputSlots, experts, N, K, gridX, gridY, 1, applyReluSquared)
}
...
func fastMXFP8MoEGatherQMMMapped(x, w, scales, counts, ids *Array, topK int, applyReluSquared bool) (y *Array, ok bool) {
	...
	gridY := (N + 63) / 64
	inputs := []C.mlx_array{x.ctx, w.ctx, scales.ctx, counts.ctx, ids.ctx}
	return applyMoEGatherMappedKernel(mxfp8MoEMappedMetalKernel, "MOE_GATHER_QMM_MAPPED", x.DType(), inputs, tokens, topK, inputSlots, experts, N, K, gridX, gridY, experts, applyReluSquared)
}
...
func fastMXFP8MoEGatherQMMBlockMapped(x, w, scales, counts, blockCount, blockExperts, blockOffsets, blockIDs *Array, topK int, applyReluSquared bool) (y *Array, ok bool) {
	...
	gridY := (N + 63) / 64
	inputs := []C.mlx_array{x.ctx, w.ctx, scales.ctx, counts.ctx, blockCount.ctx, blockExperts.ctx, blockOffsets.ctx, blockIDs.ctx}
	return applyMoEGatherMappedKernel(mxfp8MoEBlockMappedMetalKernel, "MOE_GATHER_QMM_BLOCK_MAPPED", x.DType(), inputs, tokens, topK, inputSlots, experts, N, K, gridX, gridY, 1, applyReluSquared)
}
