...
func TestReadConfigQuantOverrides_PerPathOverrideWithExplicitMode(t *testing.T) {
	...
	if info.QuantType != "AFFINE" || info.GroupSize != 64 || info.Bits != 8 || info.Mode != "affine" {
		t.Errorf("override = %+v", info)
	}
}
...
func TestReadConfigQuantOverrides_PerPathOverrideOmittedModeIsAffine(t *testing.T) {
	...
	info := overrides["foo.mlp.gate.weight"]
	if info == nil || info.Mode != "affine" || info.QuantType != "AFFINE" || info.GroupSize != 64 || info.Bits != 8 {
		t.Errorf("override = %+v, want {AFFINE,64,8,affine}", info)
	}
}
