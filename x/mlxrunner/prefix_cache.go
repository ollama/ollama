...
// effectiveKeyTokens returns the per-position key alphabet for a prompt: the
// token ID itself outside media expansions, and the item's fold value across
// each expansion's whole range. Fold values have bit 31 set, which no token
// ID does, so a media stream can never alias a text stream.
...
// key converts effective tokens to trie keys, one per restorable cache
// offset. A model that drafts through MTP-style draft caches pairs each
// cache slot with the token after it, so slot i is reusable only if token
// i+1 also matched. The key for offset i then packs (token i, token i+1):
// matching k keys verifies k+1 tokens, making every match a valid restore
// point.
