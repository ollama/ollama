...
func TestFoldValue(t *testing.T) {
	a := foldValue([]byte("image-a"), []int{4, 768})
	b := foldValue([]byte("image-b"), []int{4, 768})
	if a == b {
		t.Fatal("different bytes produced the same fold")
	}
	if a&(1<<31) == 0 || b&(1<<31) == 0 {
		t.Fatal("fold value missing bit 31")
	}
	if a != foldValue([]byte("image-a"), []int{4, 768}) {
		t.Fatal("fold not deterministic")
	}
	if a == foldValue([]byte("image-a"), []int{8, 768}) {
...
func TestBatchMediaLifecycle(t *testing.T) {
	...
	m.release(6)
	if m.manifest[0].Features != nil || m.features[0] != nil {
