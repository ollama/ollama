...
func TestChatAssistantEntryHasNoPrefixAndUsesInset(t *testing.T) {
	m := chatModel{entries: []chatEntry{{role: "assistant", content: "hello"}}}
	...
	if strings.Contains(transcript, "•") {
		t.Fatalf("assistant transcript should not include bullet: %q", transcript)
	}
	if !strings.HasPrefix(transcript, "  hello") {
		t.Fatalf("assistant transcript should use leading inset: %q", transcript)
	}
}
...
func TestChatViewRendersEmptyPromptHint(t *testing.T) {
	...
	hintLine := -1
	for _, prompt := range chatEmptyPrompts {
		if i := lineIndexContaining(lines, prompt); i >= 0 {
			hintLine = i
			break
		}
	}
...
func TestChatViewRendersInputBox(t *testing.T) {
	...
	if !strings.Contains(view, inputBoxTopBorderLine(40)) || !strings.Contains(view, inputBoxBottomBorderLine(40)) {
		t.Fatalf("prompt input should render rounded box borders: %q", view)
	}
	if !strings.Contains(view, "hello"+inputCursorGlyph) {
...
func TestChatViewRendersCursorWithEmptyPlaceholder(t *testing.T) {
	...
	rawView := m.View()
	hint := m.emptyChatHint()
	first := string([]rune(hint)[0])
	if !strings.Contains(rawView, renderInputCursorCell(first)) {
		t.Fatalf("empty placeholder should style first character as cursor: %q", rawView)
	}
	if view := stripANSI(rawView); !strings.Contains(view, hint) || strings.Contains(view, inputCursorGlyph) {
		t.Fatalf("empty placeholder should keep first character visible: %q", view)
	}
}
...
func TestChatViewRendersModelUnderInputBox(t *testing.T) {
	...
	inputLine := lineIndexContaining(lines, "hello"+inputCursorGlyph)
	...
	if modelLine != inputLine+2 {
		t.Fatalf("model line should sit directly below the input box: input=%d model=%d\n%s", inputLine, modelLine, strings.Join(lines, "\n"))
	}
...
func TestChatViewCapsTallInputBox(t *testing.T) {
	...
	if strings.Contains(view, "...") {
		t.Fatalf("truncated pasted prompt should not show ellipses:\n%s", view)
	}
}
...
func TestChatViewRendersNotificationAboveInput(t *testing.T) {
	...
	for i, line := range lines {
		if strings.Contains(line, "hello"+inputCursorGlyph) {
			if i < 2 || !strings.Contains(lines[i-2], "copied latest output") {
...
func TestChatViewKeepsInputBoxWhileRunning(t *testing.T) {
	...
	view := stripANSI(m.View())
	if !strings.Contains(view, "next"+inputCursorGlyph) {
	...
	lines := strings.Split(view, "\n")
	for i, line := range lines {
		if strings.Contains(line, "next"+inputCursorGlyph) {
			if i < 2 || !strings.Contains(lines[i-2], "Thinking 42 tokens") {
...
func TestChatViewDoesNotPadToTerminalHeight(t *testing.T) {
	...
	if !strings.Contains(stripANSI(view), "hello"+inputCursorGlyph) {
...
func TestChatViewDoesNotReserveIdleActionSpacerAfterResponse(t *testing.T) {
	...
	if gap := inputLine - assistantLine - 1; gap != 3 {
		t.Fatalf("gap between finished response and input body = %d, want 3 including stable action slot and border:\n%s", gap, strings.Join(lines, "\n"))
	}
}
...
func TestChatResizeKeepsFlowForLongAssistantOutput(t *testing.T) {
	...
	m = updated.(chatModel)
	if cmd == nil {
		t.Fatal("terminal resize should repaint flushed flow transcript")
	}
	if m.boundedFrame || m.fullScreen {
		t.Fatal("terminal resize should keep normal terminal-flow rendering")
	}
	if m.flowPrintedLines == 0 {
		t.Fatal("resize should re-flush flow transcript after repaint")
	}
	if strings.Contains(stripANSI(m.View()), "generated line 79") {
		t.Fatalf("flow resize should not recover flushed transcript into managed frame:\n%s", stripANSI(m.View()))
	}
}
...
func TestEntriesFromMessagesRendersCompactionSummaryCollapsed(t *testing.T) {
	...
	m := chatModel{boundedFrame: true, entries: entries}
...
func TestChatCtrlORepaintsDetailsInFlowMode(t *testing.T) {
	m := chatModel{
		width:            100,
		height:           20,
		flowPrintedLines: 1,
	...
	m = updated.(chatModel)
	if cmd == nil {
		t.Fatal("ctrl+o should repaint retained flow transcript details")
	}
	if m.boundedFrame || m.fullScreen {
		t.Fatal("ctrl+o should stay in terminal-flow rendering")
	}
	if !m.toolOutputOpen || !m.entries[1].expanded {
		t.Fatalf("ctrl+o should expand retained tool details in flow mode: %#v", m.entries[1])
	}
	if got, want := m.flowPrintedLines, len(m.transcriptLines(m.width)); got != want {
		t.Fatalf("flowPrintedLines = %d, want reprinted transcript line count %d", got, want)
	}
}

func TestChatCtrlOFlowReplaceKeepsInputBarVisible(t *testing.T) {
	m := chatModel{
		width:            100,
		height:           20,
		flowPrintedLines: 1,
	...
	m = updated.(chatModel)
	view := stripANSI(m.View())
	if !strings.Contains(view, inputCursorGlyph) || !strings.Contains(view, "test-model") {
		t.Fatalf("ctrl+o flow replacement should keep input surface visible: %q", view)
	}
}
...
func TestChatCtrlOPinsTranscriptToBottom(t *testing.T) {
	m := chatModel{
		width:        80,
		height:       8,
		boundedFrame: true,
		...
		entries: []chatEntry{
			{role: "user", content: "inspect this"},
			{role: "assistant", content: strings.Repeat("older line\n", 10)},
			{role: "tool", detail: "bash", label: "Bash(\"pwd\")", status: "done", content: strings.Repeat("tool output\n", 10)},
			{role: "assistant", content: "latest answer"},
		},
	}
	m.scroll = m.maxScroll()
	if m.scroll == 0 {
		t.Fatal("test setup should start scrolled away from bottom")
	}

	updated, _ := m.Update(tea.KeyMsg{Type: tea.KeyCtrlO})
	m = updated.(chatModel)
	if m.scroll != 0 {
		t.Fatalf("ctrl+o should pin transcript to bottom, got scroll %d", m.scroll)
	}
	if view := stripANSI(m.View()); !strings.Contains(view, "latest answer") {
		t.Fatalf("bottom of transcript should stay visible after ctrl+o:\n%s", view)
	}
}
...
func TestChatToolGroupStatusOmitsResultCounts(t *testing.T) {
	...
	if line != "Used 3 tools" {
		t.Fatalf("group status = %q, want generic summary", line)
	}
}
...
func TestChatToolGroupStatusShowsAllSuccessCount(t *testing.T) {
	...
	if line != "Used 2 tools" {
		t.Fatalf("group status = %q, want generic summary for all-success group", line)
	}
}

func TestChatToolGroupStatusSummarizesActions(t *testing.T) {
	entry := newChatEntry(chatEntry{
		role:   "tool_group",
		label:  "Tool calls (2)",
		status: "done",
		tools: []chatEntry{
			newChatEntry(chatEntry{role: "tool", detail: "bash", status: "done"}),
			newChatEntry(chatEntry{role: "tool", detail: "edit", status: "done"}),
		},
	})

	line := stripANSI(toolGroupStatusLine(entry))
	if line != "Ran a command and edited a file" {
...
func TestChatToolCallRendersPrettyInvocationAndResult(t *testing.T) {
	...
	m := chatModel{width: 100, height: 30, boundedFrame: true}
	m.applyAgentEvent(coreagent.Event{
		Type: coreagent.EventToolCallDetected,
		ToolCalls: []api.ToolCall{{
			ID: "call-1",
