...
func (inv *inventory) gguf(ctx context.Context, a Artifact, role, byteOrder string) error {
	f, err := gguf.Open(a.File)
	if err != nil {
		return fmt.Errorf("%s: %w", a.File, err)
	}
	defer f.Close()

	meta := make(map[string]any, f.NumKeyValues())
	for _, kv := range f.KeyValues() {
		raw, err := json.Marshal(kv.Value.Any())
		if err != nil {
			return fmt.Errorf("encode GGUF metadata %q: %w", kv.Key, err)
		}
		meta[kv.Key], err = decodeJSON(raw)
		if err != nil {
			return fmt.Errorf("decode GGUF metadata %q: %w", kv.Key, err)
		}
	}
	...
	// Alignment is container layout, and general.parameter_count is derived
	// from the tensor inventory, so omit both values from semantic metadata
	// comparison.
	...
	for _, ti := range f.TensorInfos() {
		...
		t := &Tensor{Name: ti.Name, Role: role, DType: strings.ToUpper(ti.Type.String()), Shape: ti.Shape, Bytes: numBytes, Offset: offset, Format: "gguf", MediaType: a.MediaType, ByteOrder: byteOrder, File: a.File, Layer: a.Name, Blob: a.Blob}
