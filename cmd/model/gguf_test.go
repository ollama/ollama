...
func TestGGUFEndianAndAlignment(t *testing.T) {
	for _, order := range []binary.ByteOrder{binary.LittleEndian, binary.BigEndian} {
		t.Run(order.String(), func(t *testing.T) {
			...
			write(uint32(1))
			write(uint64(2))
			write(uint32(gguf.TensorTypeF32))
			write(uint64(0))
...
func TestGGUFPayloadAndMetadata(t *testing.T) {
	var paths []string
	var wantBytesHashed int64
	for i := range 2 {
		f, err := os.CreateTemp(t.TempDir(), "*.gguf")
		if err != nil {
			t.Fatal(err)
		}
		kv := gguftest.KV{"general.architecture": "llama", "general.description": []string{"left", "right"}[i], "general.parameter_count": uint64(99 + i), "tokenizer.ggml.tokens": []string{"a", " b"}}
		tensors := []*gguftest.Tensor{{Name: "weight", Type: gguf.TensorTypeQ4_K, Shape: []uint64{256, 1}, WriterTo: bytes.NewReader(append(make([]byte, 143), byte(i)))}, {Name: "scalar", Type: gguf.TensorTypeF32, Shape: []uint64{1}, WriterTo: bytes.NewReader([]byte{0, 0, 128, 63})}}
		...
		if i == 0 {
			wantBytesHashed = int64(2 * (tensors[0].Size() + tensors[1].Size()))
		}
	}
