...
func TestTextDeterministic(t *testing.T) {
	...
	for _, s := range []string{"@@ 2 changes: all payload-only within U8; 0 descriptor changes; 0 dtype transitions; metadata semantic 0 / provenance 0 @@", "  tensor status: 2 changed", "  inventory: 2 tensors; 2 B; 1 file outside blob store", "@@ components @@", "1 change omitted", "--all", "--- tensor/model/a", "- payload sha256:", "2 tensors compared by SHA-256 (4 B read)"} {
		if !strings.Contains(out.String(), s) {
			t.Errorf("text missing %q: %s", s, out.String())
		}
	}
	for _, s := range []string{"Summary\n", "Metadata\n===", "Tensors\n===", "Proof", "proof=", "0 not checked", "0 unchecked", "Components (name-based)", "Inspecting left"} {
		if strings.Contains(out.String(), s) {
...
func TestOverviewUsesDiffInventory(t *testing.T) {
	left, right := t.TempDir(), t.TempDir()
	leftPath := tensorFixture(t, left, "model.safetensors", []fixtureTensor{{"a", "U8", []int{1}, []byte{1}}}, nil)
	rightPath := tensorFixture(t, right, "model.safetensors", []fixtureTensor{{"a", "U8", []int{1}, []byte{1}}, {"b", "U8", []int{2}, []byte{2, 3}}}, nil)
	r, err := Compare(t.Context(), leftPath, rightPath, Options{})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, want := range []string{
		"@@ 1 change; 1 descriptor change; 0 dtype transitions; metadata semantic 0 / provenance 0 @@",
		"  tensor status: 1 unchanged, 1 added",
		"- inventory: 1 tensor; 1 B; 1 file outside blob store",
		"+ inventory: 2 tensors; 3 B; 1 file outside blob store",
	} {
...
func TestMetadataUsesDiffHeaders(t *testing.T) {
	...
	for _, value := range []string{"--- metadata/config/size", "+++ metadata/config/size", "- 1", "+ 2"} {
...
func TestRenameDetection(t *testing.T) {
	...
	if !strings.Contains(out.String(), "@@ rename candidates @@") || !strings.Contains(out.String(), "same descriptor and payload sha256") {
...
func TestExpertFusionDetection(t *testing.T) {
	...
	if !strings.Contains(out.String(), "@@ expert fusion @@") || !strings.Contains(out.String(), "mapping: recognized from complete expert indices and logical shape") {
