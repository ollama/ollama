...
func writeOverview(b *strings.Builder, r *Report) {
	fmt.Fprintf(b, "@@ %s @@\n", headline(r))
	...
	s := r.Summary
	fmt.Fprintf(b, "  tensor status: %s\n", countSummary(s.Counts))
	left := inventoryText(s.Equal+s.Changed+s.Removed, s.LeftBytes, s.LeftBlobs, s.LeftFiles)
	right := inventoryText(s.Equal+s.Changed+s.Added, s.RightBytes, s.RightBlobs, s.RightFiles)
	writeDiffLine(b, "inventory", left, right)
	if s.SharedBlobs > 0 {
		fmt.Fprintf(b, "  shared blobs: %d by digest\n", s.SharedBlobs)
	}
...
func inventoryText(tensors int, bytes int64, blobs, files int) string {
	parts := []string{
		fmt.Sprintf("%d %s", tensors, plural(tensors, "tensor", "tensors")),
		ollamaformat.HumanBytes(bytes),
	}
	if blobs > 0 {
		parts = append(parts, fmt.Sprintf("%d %s", blobs, plural(blobs, "blob", "blobs")))
	}
	if files > 0 {
		parts = append(parts, fmt.Sprintf("%d %s outside blob store", files, plural(files, "file", "files")))
	}
	return strings.Join(parts, "; ")
}

func writeDiffLine(b *strings.Builder, label, left, right string) {
	if left == right {
		fmt.Fprintf(b, "  %s: %s\n", label, left)
		return
	}
...
func writeStats(b *strings.Builder, r *Report, all, summaryOnly bool, limit int) {
	...
	fmt.Fprintln(b)
	writeQuantStatsLine(b, '-', s.Left)
	writeQuantStatsLine(b, '+', s.Right)
...
func writeQuantStatsLine(b *strings.Builder, prefix byte, stats QuantStats) {
	var types []string
	...
	fmt.Fprintf(b, "%c quantization: %s", prefix, strings.Join(types, ", "))
