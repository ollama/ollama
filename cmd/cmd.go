...
func ExportHandler(cmd *cobra.Command, args []string) error {
	...
	// Write model names (one per line)
	for _, m := range models.Models {
		// Skip cloud/remote models as they can't be pulled directly
		if m.RemoteModel != "" {
	...
	if outputFile != "" {
		fmt.Fprintf(os.Stderr, "Models list saved to: %s\n", outputFile)
	}
...
func PullHandler(cmd *cobra.Command, args []string) error {
	...
	// Check if -r flag is provided for batch pull from file
	requirementsFile, _ := cmd.Flags().GetString("requirements")
	if requirementsFile != "" {
		return pullFromFile(cmd, requirementsFile, insecure)
	}

	// Normal single model pull
	if len(args) == 0 {
		return fmt.Errorf("model name required (or use -r <file> for batch pull)")
	}

	return pullModel(cmd, args[0], insecure)
}

func pullFromFile(cmd *cobra.Command, filename string, insecure bool) error {
	...
	if len(models) == 0 {
		fmt.Println("No models found in the requirements file")
		return nil
	}

	fmt.Printf("Found %d model(s) to pull\n\n", len(models))

	var failed []string
	for i, modelName := range models {
		fmt.Printf("[%d/%d] Pulling %s\n", i+1, len(models), modelName)
		if err := pullModel(cmd, modelName, insecure); err != nil {
	...
	if len(failed) > 0 {
		fmt.Fprintf(os.Stderr, "\nFailed to pull %d model(s):\n", len(failed))
		for _, m := range failed {
			fmt.Fprintf(os.Stderr, "  - %s\n", m)
		}
		return fmt.Errorf("failed to pull %d model(s)", len(failed))
	}

	fmt.Printf("\nSuccessfully pulled all %d model(s)\n", len(models))
	return nil
}

func pullModel(cmd *cobra.Command, modelName string, insecure bool) error {
	client, err := api.ClientFromEnvironment()
	if err != nil {
		return err
	}
...
func NewCLI() *cobra.Command {
	...
	pullCmd := &cobra.Command{
		...
		Args: func(cmd *cobra.Command, args []string) error {
			// Allow 0 args if -r flag is provided
			requirementsFile, _ := cmd.Flags().GetString("requirements")
			if requirementsFile != "" {
				return nil
			}
			if len(args) < 1 {
				return fmt.Errorf("requires a model name argument (or use -r <file> for batch pull)")
			}
			return nil
		},
		PreRunE: checkServerHeartbeat,
	...
	pullCmd.Flags().StringP("requirements", "r", "", "Pull models from a requirements file (generated by 'ollama dump')")
