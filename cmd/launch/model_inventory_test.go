...
func TestModelInventoryResolveDoesNotRefreshCloudMiss(t *testing.T) {
	...
	got := inventory.Resolve(context.Background(), []string{"glm-5.1:cloud", "glm-5.2:cloud"})
	if calls != 1 {
		t.Fatalf("List calls = %d, want 1", calls)
	}
	if len(got) != 2 {
		t.Fatalf("Resolve returned %d models, want 2", len(got))
	}
	...
	if got[1].Name != "glm-5.2:cloud" || !got[1].Remote {
		t.Fatalf("resolved model = %#v, want cloud fallback", got[1])
	}
	if got[1].ContextLength <= 0 || got[1].MaxOutputTokens <= 0 {
		t.Fatalf("cloud limits not applied: %#v", got[1])
	}
}
