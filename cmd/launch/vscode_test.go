...
func TestIsVSCodeBinary(t *testing.T) {
	v := &VSCode{}

	t.Run("symlink to VS Code app", func(t *testing.T) {
		tmpDir := t.TempDir()
		target := filepath.Join(tmpDir, "Visual Studio Code.app", "Contents", "Resources", "app", "bin", "code")
		os.MkdirAll(filepath.Dir(target), 0o755)
		os.WriteFile(target, []byte("#!/bin/bash\n"), 0o755)

		link := filepath.Join(tmpDir, "code")
		os.Symlink(target, link)

		if !v.isVSCodeBinary(link) {
			t.Error("expected symlink into Visual Studio Code.app to be accepted")
		}
	})

	t.Run("symlink to Cursor app", func(t *testing.T) {
		tmpDir := t.TempDir()
		target := filepath.Join(tmpDir, "Cursor.app", "Contents", "Resources", "app", "bin", "code")
		os.MkdirAll(filepath.Dir(target), 0o755)
		os.WriteFile(target, []byte("#!/bin/bash\n"), 0o755)
		...
		if v.isVSCodeBinary(link) {
			t.Error("expected symlink into Cursor.app to be rejected")
		}
	})

	t.Run("wrapper script referencing VS Code", func(t *testing.T) {
		tmpDir := t.TempDir()
		script := filepath.Join(tmpDir, "code")
		os.WriteFile(script, []byte("#!/bin/bash\nexec /usr/share/code/bin/code \"$@\"\n"), 0o755)

		if !v.isVSCodeBinary(script) {
			t.Error("expected wrapper script with /share/code/ to be accepted")
		}
	})

	t.Run("wrapper script referencing Cursor", func(t *testing.T) {
		tmpDir := t.TempDir()
		script := filepath.Join(tmpDir, "code")
		os.WriteFile(script, []byte("#!/bin/bash\nexec /opt/cursor/bin/cursor \"$@\"\n"), 0o755)

		if v.isVSCodeBinary(script) {
			t.Error("expected wrapper script referencing Cursor to be rejected")
		}
	})

	t.Run("large binary is not read", func(t *testing.T) {
		tmpDir := t.TempDir()
		bin := filepath.Join(tmpDir, "code")
		// Write a file larger than 4096 bytes that contains a marker —
		// it should NOT be read.
		content := make([]byte, 5000)
		copy(content, []byte("Visual Studio Code"))
		os.WriteFile(bin, content, 0o755)

		if v.isVSCodeBinary(bin) {
			t.Error("expected large file to be skipped")
		}
	})

	t.Run("nonexistent path", func(t *testing.T) {
		if v.isVSCodeBinary("/nonexistent/code") {
