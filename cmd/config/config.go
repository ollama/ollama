...
// updateIntegration loads the integration for appName (an empty record if
// unset), applies mutate, and saves. Fields the mutator does not touch are
// preserved.
func updateIntegration(appName string, mutate func(*integration)) error {
	if appName == "" {
...
func SaveIntegration(appName string, models []string) error {
	return updateIntegration(appName, func(i *integration) {
		i.Models = models
	})
}
...
// Aliases are replaced entirely (not merged) so deletions are persisted.
