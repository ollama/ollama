...
func TestMoltbotIntegration(t *testing.T) {
	c := &Moltbot{}

	t.Run("String", func(t *testing.T) {
		if got := c.String(); got != "Moltbot" {
			t.Errorf("String() = %q, want %q", got, "Moltbot")
		}
	})
...
func TestMoltbotEdit(t *testing.T) {
	c := &Moltbot{}
	tmpDir := t.TempDir()
	...
	t.Run("fresh install", func(t *testing.T) {
		...
		assertMoltbotModelExists(t, configPath, "llama3.2")
		assertMoltbotPrimaryModel(t, configPath, "ollama/llama3.2")
	})
	...
	t.Run("multiple models - first is primary", func(t *testing.T) {
		...
		assertMoltbotModelExists(t, configPath, "llama3.2")
		assertMoltbotModelExists(t, configPath, "mistral")
		assertMoltbotPrimaryModel(t, configPath, "ollama/llama3.2")
	})
	...
	t.Run("edit replaces models list", func(t *testing.T) {
		...
		assertMoltbotModelExists(t, configPath, "llama3.2")
		assertMoltbotModelNotExists(t, configPath, "mistral")
	})
...
func TestMoltbotModels(t *testing.T) {
	c := &Moltbot{}
	tmpDir := t.TempDir()
	setTestHome(t, tmpDir)
...
// Helper functions
func assertMoltbotModelExists(t *testing.T, path, model string) {
	t.Helper()
...
func assertMoltbotModelNotExists(t *testing.T, path, model string) {
	t.Helper()
	data, _ := os.ReadFile(path)
	var cfg map[string]any
	json.Unmarshal(data, &cfg)
...
func assertMoltbotPrimaryModel(t *testing.T, path, expected string) {
	t.Helper()
	data, _ := os.ReadFile(path)
	var cfg map[string]any
	json.Unmarshal(data, &cfg)
...
func TestMoltbotPaths(t *testing.T) {
	c := &Moltbot{}
...
func TestMoltbotModelsEdgeCases(t *testing.T) {
	c := &Moltbot{}
	tmpDir := t.TempDir()
	setTestHome(t, tmpDir)
	configDir := filepath.Join(tmpDir, ".clawdbot")
...
func TestMoltbotEditSchemaFields(t *testing.T) {
	c := &Moltbot{}
	tmpDir := t.TempDir()
	setTestHome(t, tmpDir)
...
func TestMoltbotEditModelNames(t *testing.T) {
	c := &Moltbot{}
	tmpDir := t.TempDir()
	setTestHome(t, tmpDir)
	configPath := filepath.Join(tmpDir, ".clawdbot", "clawdbot.json")
	...
	t.Run("model with colon tag", func(t *testing.T) {
		...
		assertMoltbotModelExists(t, configPath, "llama3.2:70b")
		assertMoltbotPrimaryModel(t, configPath, "ollama/llama3.2:70b")
	})
	...
	t.Run("model with slash", func(t *testing.T) {
		...
		assertMoltbotModelExists(t, configPath, "library/model:tag")
		assertMoltbotPrimaryModel(t, configPath, "ollama/library/model:tag")
	})
	...
	t.Run("model with hyphen", func(t *testing.T) {
		...
		assertMoltbotModelExists(t, configPath, "test-model")
	})
}

func TestMoltbotEditAgentsPreservation(t *testing.T) {
	c := &Moltbot{}
	tmpDir := t.TempDir()
	setTestHome(t, tmpDir)
	configDir := filepath.Join(tmpDir, ".clawdbot")
...
func TestMoltbotEdit_RoundTrip(t *testing.T) {
	c := &Moltbot{}
	tmpDir := t.TempDir()
	setTestHome(t, tmpDir)
	configDir := filepath.Join(tmpDir, ".clawdbot")
	...
	os.MkdirAll(configDir, 0o755)
	os.WriteFile(configPath, []byte(testMoltbotFixture), 0o644)
...
func TestMoltbotEdit_Idempotent(t *testing.T) {
	c := &Moltbot{}
	tmpDir := t.TempDir()
	setTestHome(t, tmpDir)
	configDir := filepath.Join(tmpDir, ".clawdbot")
...
func TestMoltbotEdit_MultipleConsecutiveEdits(t *testing.T) {
	c := &Moltbot{}
	tmpDir := t.TempDir()
	setTestHome(t, tmpDir)
	configDir := filepath.Join(tmpDir, ".clawdbot")
...
func TestMoltbotEdit_BackupCreated(t *testing.T) {
	c := &Moltbot{}
	tmpDir := t.TempDir()
	setTestHome(t, tmpDir)
	configDir := filepath.Join(tmpDir, ".clawdbot")
...
func TestMoltbotEdit_CreatesDirectoryIfMissing(t *testing.T) {
	c := &Moltbot{}
	tmpDir := t.TempDir()
	setTestHome(t, tmpDir)
	configDir := filepath.Join(tmpDir, ".clawdbot")
