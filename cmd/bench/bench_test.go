...
type mockServerOptions struct {
	...
	// failTokenCount makes DebugRenderOnly requests return 404, simulating
	// a server without input token counting.
	failTokenCount bool
}

// mockRenderedTokens deterministically "counts" a chat request the way the
// server's rendered input token count would: ~1.3 tokens/word of content plus
// template overhead.
func mockRenderedTokens(req api.ChatRequest) int {
	words := 0
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/chat":
			...
			if req.DebugRenderOnly {
				if opts.failTokenCount {
					http.Error(w, "Not found", http.StatusNotFound)
					return
				}
				tokens := mockRenderedTokens(req)
...
func TestBenchmarkModel_TokenCountUnsupported(t *testing.T) {
	fOpt := createTestFlagOptions()
	promptTokens := 2000
	fOpt.promptTokens = &promptTokens
	...
	output := captureOutput(func() {
		err := BenchmarkModel(fOpt)
		if err == nil {
			t.Error("Expected error when server cannot count prompt tokens")
		}
	})

	if !strings.Contains(output, "cannot count prompt tokens") {
		t.Errorf("Expected token counting error, got: %s", output)
	}
}
...
func TestBenchmarkModel_ShortResponseRetry(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/chat":
			...
			if req.DebugRenderOnly {
				t.Error("Unexpected token count request without promptTokens")
			}
