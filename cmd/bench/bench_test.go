...
func TestBenchmarkModel_PrefillExcludesCachedTokens(t *testing.T) {
	...
	format := "csv"
	fOpt.format = &format
	responses := defaultChatResponses()
	responses[len(responses)-1].Metrics.PromptEvalCachedCount = testIntPtr(4)
	...
	wantPrefix := fmt.Sprintf("test-model,prefill,%d,", responses[len(responses)-1].Metrics.PromptEvalCount-*responses[len(responses)-1].Metrics.PromptEvalCachedCount)
...
func TestOutputFormatHeader(t *testing.T) {
	tests := []struct {
		...
		{"csv", false, []string{"NAME,STEP,COUNT,NS_PER_COUNT,TOKEN_PER_SEC,CACHED_PROMPT_COUNT"}},
