...
func TestValidateRecurrentSSMInAccepted(t *testing.T) {
	// When SSMIn is set, Validate must not reject the layer for missing attn_qkv/attn_gate.
	// It will still fail on a later check (e.g. missing ssm_dt), but that is expected.
	m := &Model{
		Layers: []Layer{{
			Operator: &GatedDeltaNet{
				SSMIn:    &nn.Linear{},
