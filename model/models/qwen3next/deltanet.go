...
type GatedDeltaNet struct {
	// Optimized path: pre-split QKV and gate (Ollama-native GGUFs)
	SSMQKV     *nn.Linear `gguf:"attn_qkv"`  // -> Q, K, V (concatenated)
	SSMQKVGate *nn.Linear `gguf:"attn_gate"` // -> Z gate
	// Combined path: Q, K, V, Z interleaved per K-head (llama.cpp imported GGUFs)
...
func (gdn *GatedDeltaNet) Forward(ctx ml.Context, hiddenStates, _ ml.Tensor, cache *HybridCache, opts *Options) (ml.Tensor, error) {
	...
	case gdn.SSMQKV != nil && gdn.SSMQKVGate != nil:
		// Ollama-native GGUFs: pre-split attn_qkv and attn_gate tensors.
	...
	case gdn.SSMIn != nil:
		// llama.cpp imported GGUFs: ssm_in packs Q,K,V,Z interleaved per K-head.
		// Layout per K-head j: [Q(headKDim) | K(headKDim) | V(vPerHead) | Z(vPerHead)]
