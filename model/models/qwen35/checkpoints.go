...
func (c *HybridCache) PrepareRestore(seq int, targetPos int32) (int32, bool) {
	...
	store, ok := c.checkpoints[slot]
	if !ok {
		slog.Debug("qwen35: checkpoint miss", "seq", seq, "slot", slot, "target", targetPos, "size", 0)
		return 0, false
	}
	idx, pos, ok := store.bestIndex(targetPos)
	if !ok {
		size, minPos, maxPos, lastPos := store.window()
		slog.Debug("qwen35: checkpoint miss", "seq", seq, "slot", slot, "target", targetPos, "size", size,
