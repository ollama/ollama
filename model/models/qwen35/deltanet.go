...
type GatedDeltaNet struct {
	...
	SSMQKV       *nn.Linear  `gguf:"attn_qkv,alt:wqkv"`         // -> Q, K, V (concatenated)
	SSMQKVGate   *nn.Linear  `gguf:"attn_gate,alt:wqkv_gate"`   // -> Z gate
	SSMIn        *nn.Linear  `gguf:"ssm_in"`                    // legacy combined QKVZ
	SSMBetaAlpha *nn.Linear  `gguf:"ssm_ba,alt:ssm_beta_alpha"` // -> beta, alpha
...
func (gdn *GatedDeltaNet) splitQKVZ(
	...
	if numKHeads == 0 || numVHeads == 0 || headKDim == 0 || headVDim == 0 {
		return nil, nil, errors.New("qwen35: invalid linear attention dimensions for ssm_in split")
	}
	if numVHeads%numKHeads != 0 {
		return nil, nil, errors.New("qwen35: num_value_heads must be divisible by num_key_heads for ssm_in split")
	}
...
func (gdn *GatedDeltaNet) Forward(ctx ml.Context, hiddenStates, _ ml.Tensor, cache *HybridCache, opts *Options) (ml.Tensor, error) {
	...
	if gdn.SSMBetaAlpha == nil {
		return nil, errors.New("qwen35: missing ssm_ba (beta/alpha projection)")
	}
	...
	default:
		return nil, errors.New("qwen35: missing attn_qkv/attn_gate or legacy ssm_in projections")
	}
	...
	if err != nil {
		...
		slog.Warn("qwen35: failed to get conv state, using zeros", "layer", layer, "error", err)
	...
	if err != nil {
		...
		slog.Warn("qwen35: failed to get delta state, using zeros", "layer", layer, "error", err)
