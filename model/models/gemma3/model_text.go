...
func setImageEmbeddings(ctx ml.Context, hiddenState ml.Tensor, multimodal []input.MultimodalIndex) []int {
	...
	for _, image := range multimodal {
		...
		if embedding == nil {
			...
			visionOutputs := embedding.View(ctx, src*embedding.Stride(0), []int{length * embedding.Dim(1)}, nil)
			ctx.Forward(visionOutputs.Copy(ctx, hiddenState.View(ctx, dst*hiddenState.Stride(0), []int{length * hiddenState.Dim(1)}, nil)))
