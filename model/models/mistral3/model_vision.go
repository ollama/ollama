...
func (m *VisionModel) positionalEmbedding(ctx ml.Context, positionIDs ml.Tensor) ml.Tensor {
	...
	freqs := make([]float32, m.headDim/2)
	for i := range freqs {
		freqs[i] = float32(1 / math.Pow(float64(m.ropeBase), float64(i)*2/float64(m.headDim)))
	}

	freqsH := make([]float32, len(freqs)/2*maxPatchesPerSide)
	freqsW := make([]float32, len(freqs)/2*maxPatchesPerSide)
	for i := range freqs {
		for j := range maxPatchesPerSide {
			if i%2 == 0 {
				freqsH[i/2*maxPatchesPerSide+j] = freqs[i] * float32(j)
			} else {
				freqsW[i/2*maxPatchesPerSide+j] = freqs[i] * float32(j)
			}
		}
	}

	h, err := ctx.Input().FromFloatSlice(freqsH, maxPatchesPerSide, len(freqs)/2)
	if err != nil {
		panic(err)
	}

	w, err := ctx.Input().FromFloatSlice(freqsW, maxPatchesPerSide, len(freqs)/2)
	if err != nil {
		panic(err)
	}
	...
	h = h.Reshape(ctx, len(freqs)/2, maxPatchesPerSide, maxPatchesPerSide).Permute(ctx, 0, 2, 1, 3).Contiguous(ctx)
	w = w.Stack(ctx, 2, slices.Repeat([]ml.Tensor{w}, maxPatchesPerSide-1)...)

	invFreqs := h.Concat(ctx, w, 0).Reshape(ctx, len(freqs), maxPatchesPerSide*maxPatchesPerSide)
	invFreqs = invFreqs.Concat(ctx, invFreqs, 0)
	return invFreqs.Rows(ctx, positionIDs)
}
