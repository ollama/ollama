...
// definitions in the system prompt are byte-identical to what the official
// Qwen 3.5 Jinja2 template produces when run through HuggingFace Transformers'
// apply_chat_template.
//
// Ground truth: The official template does {{ tool | tojson }}. HuggingFace
// Transformers overrides Jinja2's default tojson filter (chat_template_utils.py
// lines 463-466) with json.dumps(sort_keys=False, ensure_ascii=False) — no key
// sorting and no HTML escaping. llama.cpp's Jinja2 engine matches (insertion
// order preserved via as_ordered_object() at value.cpp:1330, sort_keys=true
// throws not_implemented_exception at value.cpp:176). Stock Jinja2's tojson
// does the opposite (sort_keys=True, HTML-escaping), but neither major inference
// framework uses the stock filter.
//
// The expected JSON was generated by running HuggingFace Transformers v5.2.0's
// _compile_jinja_template on the official Qwen/Qwen3.5-27B chat_template.jinja
// with the exact tool definitions below. Every byte has been verified.
//
...
//
//  2. Field ordering in ToolFunctionParameters: "properties" before "required".
//     Go's json.Marshal emits struct fields in declaration order. The
//     ToolFunctionParameters struct must declare Properties before Required to
//     match every Python/Jinja2 configuration (both insertion-order-preserving
//     and alphabetical, since p < r).
//
//  3. Field ordering in map[string]any sub-objects (items): "type" before
//     "description". Go's json.Marshal alphabetizes map keys, producing
//     "description" before "type". Both HuggingFace Transformers (insertion
//     order) and llama.cpp (insertion order) produce "type" before "description".
//     This requires an ordered map type for the Items field — not yet fixed.
func TestQwen35RendererToolDefinitionsMatchOfficialTemplate(t *testing.T) {
	...
	// The official template's tojson (via HuggingFace Transformers' override
	// with ensure_ascii=False) outputs these characters literally. A tool
	// description like "Returns temperature in <fahrenheit> & <celsius>"
	// becomes completely different tokens when HTML-escaped — 379 bytes vs
	// 404 bytes for the full tool JSON, with every subsequent token ID shifted.
	for _, esc := range []string{"\\u003c", "\\u003e", "\\u0026"} {
		if strings.Contains(got, esc) {
			t.Errorf(
				"HTML-escaped sequence %q found in tool definition.\n\n"+
					"HuggingFace Transformers overrides Jinja2's tojson filter with "+
					"json.dumps(ensure_ascii=False) at chat_template_utils.py:463-466. "+
					"llama.cpp's tojson writes literal characters for all printable "+
					"ASCII (value.cpp:1302-1309). Both produce literal <, >, & — not "+
					"Unicode escape sequences. Go's default json.Marshal HTML-escapes "+
					"these characters; the fix is jsonutil.Marshal with SetEscapeHTML(false).\n\n"+
					"got:\n%s", esc, got,
			)
		}
	}

	// Property 2: Field ordering — "properties" before "required" in
	// ToolFunctionParameters, and correct key ordering throughout.
	//
	// The training data format is determined by HuggingFace Transformers'
	// get_json_schema() function, which produces Python dicts with a
	// specific insertion order. Go's json.Marshal emits struct fields in
	// declaration order, normalizing all client inputs to the struct field
	// order regardless of what key order the client sent. This is correct
	// as long as the Go struct field order matches get_json_schema()'s
	// insertion order:
	...
	//
	// NOTE: The items sub-object below uses {"type", "description"} which
	// HuggingFace Transformers' get_json_schema() never produces inside
	// items — description is only added to top-level properties. This test
	// catches the real key-ordering bug (Go alphabetizes map[string]any
	// keys) but with an out-of-training-distribution example. Needs
	// rewriting to use get_json_schema() examples like list[dict[str, int]]
	// which produce real multi-key items such as
	...
	want := `{"type": "function", "function": {"name": "get_weather", "description": "Returns temperature in <fahrenheit> & <celsius>", "parameters": {"type": "object", "properties": {"location": {"type": "string", "description": "City name with <tag> & symbol"}, "filters": {"type": "array", "items": {"type": "string", "description": "Use < and > literally & keep order"}}}, "required": ["location"]}}}`
	if !strings.Contains(got, want) {
		// Extract the actual tool JSON line for a precise diff
		actualToolJSON := ""
		for _, line := range strings.Split(got, "\n") {
			if strings.Contains(line, `"type": "function"`) && strings.Contains(line, `"get_weather"`) {
				actualToolJSON = line
				break
			}
		}
		t.Errorf(
			"tool definition JSON does not match the training data format.\n\n"+
				...
				"Qwen/Qwen3.5-27B Jinja2 chat template. Go's struct-based json.Marshal "+
				"normalizes all client inputs to the struct field order, which must match "+
				"get_json_schema()'s insertion order. Any deviation means the model sees "+
				"different token IDs for the system prompt, pushing it out of the training "+
				"distribution.\n\n"+
				"Common causes:\n"+
				"  - 'required' before 'properties': ToolFunctionParameters struct field order wrong\n"+
				"  - alphabetized keys in items: Items typed as any, parsed as map[string]any\n"+
				"  - HTML-escaped <, >, &: json.Marshal default; need SetEscapeHTML(false)\n\n"+
				"expected tool JSON:\n%s\n\nactual tool JSON:\n%s\n\nfull rendered output:\n%s",
			want, actualToolJSON, got,
		)
	}
}
