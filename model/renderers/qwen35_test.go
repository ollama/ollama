...
func TestQwen35RendererToolDefinitionsMatchOfficialTemplate(t *testing.T) {
	...
	//
	// This is the byte-exact expected output from HuggingFace Transformers
	// v5.2.0 running the official Qwen/Qwen3.5-27B chat_template.jinja with
	// the exact tool definitions above. The key ordering comes from Python
	// dict insertion order (preserved by HF's sort_keys=False override):
	//   - Tool level: "type" -> "function" (get_json_schema return order)
	//   - Function level: "name" -> "description" -> "parameters"
	//   - Parameters level: "type" -> "properties" -> "required"
	//   - Property level: "type" -> "description"
	//   - Items level: "type" -> "description"
	//
	...
	if !strings.Contains(got, want) {
		...
		t.Errorf(
			"tool definition JSON does not match the official Qwen 3.5 template output.\n\n"+
				"The expected JSON was generated by running HuggingFace Transformers v5.2.0 "+
				"_compile_jinja_template on the official Qwen/Qwen3.5-27B chat_template.jinja. "+
				"Any byte-level deviation means the model sees different token IDs for the "+
				"system prompt, pushing the input out of the training distribution. In multi-turn "+
				"agentic conversations, the entire system prompt is re-rendered on every turn, "+
				"so a deviation here invalidates the KV cache from the system message onward.\n\n"+
				"Common causes:\n"+
				"  - 'required' before 'properties': ToolFunctionParameters struct field order wrong\n"+
				"  - 'description' before 'type' in items: Go alphabetizes map[string]any keys\n"+
