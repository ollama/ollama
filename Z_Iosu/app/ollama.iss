; Inno Setup Installer for Ollama (Z_Iosu customizations)
;
; To build the installer use the build script invoked from the top of the source tree
; 
; powershell -ExecutionPolicy Bypass -File .\Z_Iosu\scripts\build_windows.ps1
...
Source: "..\dist\vulkan_runtime_installer.exe"; DestDir: "{tmp}"; Check: not IsArm64() and VulkanRuntimeNeeded(); Flags: deleteafterinstall
...
#endif
Filename: "{tmp}\vulkan_runtime_installer.exe"; Parameters: "/S"; Check: not IsArm64() and VulkanRuntimeNeeded() and FileExists(ExpandConstant('{tmp}\vulkan_runtime_installer.exe')); StatusMsg: "Installing Vulkan Runtime..."; Flags: waituntilterminated runhidden
...
begin
  if FileExists(ExpandConstant('{sys}\vulkan-1.dll')) then
    Result := False
  else if IsWin64 and FileExists(ExpandConstant('{syswow64}\vulkan-1.dll')) then
    Result := False
  else
    Result := True;
end;
