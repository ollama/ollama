...
# Uso: powershell -ExecutionPolicy Bypass -File Z_Iosu\scripts\build_app_msvc.ps1
...
Write-Host ""
Write-Host "[2/4] Inicializando entorno Visual Studio 2022..." -ForegroundColor Yellow
...
Write-Host "  OK MSVC 2022: $MSVC_INSTALL" -ForegroundColor Green

Write-Host ""
Write-Host "[3/4] Generando recursos y compilando..." -ForegroundColor Yellow
...
# CRITICO: Limpiar variables de entorno de llvm-mingw para usar MSVC puro
...
if ($buildResult -ne 0) {
    Write-Error "Compilacion fallo con codigo de salida: $buildResult"
    exit 1
}
...
if (Test-Path $appPath) {
    ...
    Write-Host ""
    Write-Host "  Archivo: $appPath" -ForegroundColor Cyan
    
    # Verificar que NO se uso llvm-mingw
    Write-Host ""
    Write-Host "  Verificando compilador usado..." -ForegroundColor Yellow
