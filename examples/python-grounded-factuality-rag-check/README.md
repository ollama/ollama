...
This example allows the user to ask questions related to a document, which can be specified via an article url. Relevant chunks are retreived from the document and given to `llama3.3` as context to answer the question. Then each sentence in the answer is checked against the retrieved chunks using `bespoke-minicheck` to ensure that the answer does not contain hallucinations. 

## Running the Example

1. Ensure `all-minilm` (embedding) `llama3.3` (chat) and `bespoke-minicheck` (check) models installed:
