...
func (c *Client) WaitUntilRunning(ctx context.Context) error {
	...
	for {
		...
		if time.Now().After(stallTimer) {
			...
			if detail != "" {
				return fmt.Errorf("timed out waiting for mlx runner to start at progress %0.2f: %s", loadProgress, detail)
			}
			return fmt.Errorf("timed out waiting for mlx runner to start at progress %0.2f", loadProgress)
		}
		...
		if progress := max(status.Progress, loadProgress); progress != loadProgress {
			...
			slog.Debug("model load progress", "progress", loadProgress)
...
// Loading status is deliberately ignored so it cannot replace the scheduler's
// context and memory estimates with partial values.
