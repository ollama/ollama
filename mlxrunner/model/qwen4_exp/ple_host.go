...
func openHostPLETables(m *manifest.Manifest, cfg *Config) (map[string]*hostPLETable, error) {
	...
	layers := make(map[string]manifest.Layer)
	...
	for _, id := range cfg.PLELayerIDs {
		...
		table, err := openHostPLETable(layers, prefix, int(cfg.SplitNGramParts), rowWidth)
...
func openHostPLETable(layers map[string]manifest.Layer, prefix string, shardCount, rowWidth int) (*hostPLETable, error) {
	table := &hostPLETable{
	...
	for i := range shardCount {
		...
		shard, err := openHostPLEShard(path, name, rowWidth)
		if err != nil {
			table.close()
