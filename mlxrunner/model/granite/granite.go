...
func init() {
	model.Register("GraniteForCausalLM", newModel)
}
...
type Config struct {
	...
	// AttentionMultiplier which defaults to 1.0 for robustness; real
	// Granite configs set it explicitly, e.g. 0.015625).
	...
	// RoPE dual-key: transformers has migrated from rope_scaling to
	// rope_parameters; prefer rope_parameters when both are present.
...
// parseConfig unmarshals and validates config JSON, applying Granite-specific
// defaults and rope dual-key handling. Mirrors laguna.go's parseConfig pattern
// so callers can test config parsing without a full model.Root.
func parseConfig(configData []byte) (Config, error) {
	...
	// Only build YaRN frequencies when the resolved type name is "yarn"
	// (case-insensitive). Any other value (including "default", "", "none",
	// or unrecognized strings) is a no-op — leave RopeFreqs=nil, RopeMScale=1.
...
func (m *Model) LoadWeights(tensors map[string]*mlx.Array) error {
	...
	if m.TieWordEmbeddings {
		...
		// Fallback used by many Granite checkpoints where output is tied.
		m.LMHead = m.EmbedTokens.AsLinear()
	}
