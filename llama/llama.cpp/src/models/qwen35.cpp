...
ggml_tensor * llm_build_qwen35::build_layer_attn_linear(
    ...
    ggml_tensor * mixed_qkv = build_lora_mm(model.layers[il].wqkv, cur);
    cb(mixed_qkv, "linear_attn_mixed_qkv", il);

    ggml_tensor * z_proj = build_lora_mm(model.layers[il].wqkv_gate, cur);
    cb(z_proj, "linear_attn_z_proj", il);

    ggml_tensor * beta_proj = build_lora_mm(model.layers[il].ssm_beta, cur);
    cb(beta_proj, "linear_attn_beta_proj", il);

    ggml_tensor * alpha_proj = build_lora_mm(model.layers[il].ssm_alpha, cur);
    cb(alpha_proj, "linear_attn_alpha_proj", il);

    int64_t       qkv_new_dim        = 2 * head_k_dim + head_v_dim * (num_v_heads / num_k_heads);
    ggml_tensor * mixed_qkv_reshaped = ggml_reshape_4d(ctx0, mixed_qkv, qkv_new_dim, num_k_heads, n_seq_tokens, n_seqs);

    ggml_tensor * beta  = ggml_reshape_3d(ctx0, beta_proj, num_v_heads, n_seq_tokens, n_seqs);
    ggml_tensor * alpha = ggml_reshape_3d(ctx0, alpha_proj, num_v_heads, n_seq_tokens, n_seqs);
    ...
    ggml_tensor * query =
        ggml_view_4d(ctx0, mixed_qkv_reshaped, split_sizes_qkv[0], num_k_heads, n_seq_tokens, n_seqs,
                     mixed_qkv_reshaped->nb[1], mixed_qkv_reshaped->nb[2], mixed_qkv_reshaped->nb[3], 0);
    cb(query, "q", il);

    ggml_tensor * key = ggml_view_4d(ctx0, mixed_qkv_reshaped, split_sizes_qkv[1], num_k_heads, n_seq_tokens, n_seqs,
                                     mixed_qkv_reshaped->nb[1], mixed_qkv_reshaped->nb[2], mixed_qkv_reshaped->nb[3],
                                     split_sizes_qkv[0] * ggml_element_size(mixed_qkv_reshaped));
    cb(key, "k", il);

    ggml_tensor * value =
        ggml_view_4d(ctx0, mixed_qkv_reshaped, split_sizes_qkv[2], num_k_heads, n_seq_tokens, n_seqs,
                     mixed_qkv_reshaped->nb[1], mixed_qkv_reshaped->nb[2], mixed_qkv_reshaped->nb[3],
                     (split_sizes_qkv[0] + split_sizes_qkv[1]) * ggml_element_size(mixed_qkv_reshaped));
    cb(value, "v", il);

    ggml_tensor * z = ggml_reshape_4d(ctx0, z_proj,
    ...
    beta = ggml_cont_4d(ctx0, ggml_reshape_4d(ctx0, beta_proj, num_v_heads, 1, n_seq_tokens, n_seqs),
                         num_v_heads, 1, n_seq_tokens, n_seqs);
