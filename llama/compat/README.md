...
| `olmo3` | Arch rename `olmo3`→`olmo2` plus KV-prefix copy. llama.cpp's OLMo2 loader covers the OLMo3 tensor layout and sliding-window RoPE behavior. | n/a |
...
| `qwen35` | Same fixes as `qwen35moe` (head_count_kv array→scalar, rope dimension_sections pad 3→4, `ssm_dt`→`ssm_dt.bias`, drop `v.*`/`mm.*`/`mtp.*`) but for the non-MoE qwen3.5 (e.g. 9B). Both arches share `apply_qwen35_text_fixes`. | Same qwen3vl-merger translation path as `qwen35moe`, using the `qwen35.*` KV prefix. |
...
| `nemotron_h_omni` | Arch rewrite to `nemotron_h_moe`/`nemotron_h`, copy text KVs to the selected prefix, drop `a.*`/`v.*`/`mm.*` tensors. Audio is intentionally hidden until upstream llama.cpp can skip or load the Nemotron audio projector safely. | Arch rewrite to `clip`, KV synthesis for `clip.vision.*` + `projector_type=nemotron_v2_vl`, disable audio, rename `v.cls_embd`→`v.class_embd`, downsample `v.position_embd`→`v.position_embd.weight`, reshape `v.patch_embd.weight`, F32 promote class/position/patch embeddings, rename `mm.{norm,1,2}` to `mm.model.mlp.{0,1,3}` |
