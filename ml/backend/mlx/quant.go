...
func gguf_load_quantized(data unsafe.Pointer, name string, shape []C.int, dtype uint32, stream C.mlx_stream) (*Array, error) {
	...
	if dtype == 2 || dtype == 3 {
		...
		return nil, fmt.Errorf("unsupported tensor type %d", dtype)
	}

	weights_per_block := C.int(32)
	if shape[len(shape)-1]%weights_per_block != 0 {
		return nil, fmt.Errorf("[load_gguf] tensor has incompatible last dim shape: %d", shape[len(shape)-1])
	}
	...
	return &Array{a: r, name: name}, nil
}

func load_k_quantized(data unsafe.Pointer, name string, shape []C.int, dtype uint32, stream C.mlx_stream) (*Array, error) {
	if dtype != 14 {
		return nil, fmt.Errorf("unsupported K quant")
	}
