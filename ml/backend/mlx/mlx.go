...
func New(r *os.File, params ml.BackendParams) (ml.Backend, error) {
	...
	for _, t := range meta.Tensors().Items() {
		g.Go(func() error {
			...
			// TODO More quantization types if possible...
			...
			case 2, 8: // GGML_TYPE_Q4_0, GGML_TYPE_Q8_0
				...
				kqv, err := gguf_load_quantized(cbytes, t.Name, shape, t.Kind, stream)
				if err != nil {
					panic(err.Error())
				}
				if strings.HasSuffix(t.Name, "attn_q.weight") || strings.HasSuffix(t.Name, "attn_q.bias") || strings.HasSuffix(t.Name, "attn_k.weight") || strings.HasSuffix(t.Name, "attn_k.bias") {
					shape := make([]C.int, C.mlx_array_ndim(kqv.a))
					for i := range shape {
						shape[i] = C.mlx_array_dim(kqv.a, C.int(i))
					}
					err := unmutate(kqv.name, shape, kqv.a)
					if err != nil {
						return err
					}
					...
					defer C.mlx_array_free(kqv.a)
					kqv.a = r
					mu.Lock()
					defer mu.Unlock()
					C.mlx_vector_array_append_value(vec, kqv.a)
					tensors[kqv.name] = kqv
				}
				return nil
			case 14: // GGML_TYPE_Q6_K
				// TODO any special cases?
				res, err := load_k_quantized(cbytes, t.Name, shape, t.Kind, stream)
				if err != nil {
					panic(err)
				}
				var r C.mlx_array
				...
				defer C.mlx_array_free(res.a)
				res.a = r
				mu.Lock()
				defer mu.Unlock()
				C.mlx_vector_array_append_value(vec, res.a)
				tensors[res.name] = res
				return nil
			default:
			...
			// TODO is there ever a case where we want to use fp32?
