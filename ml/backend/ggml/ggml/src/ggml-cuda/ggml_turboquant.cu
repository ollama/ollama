...
template<int N>
__device__ inline void wht_butterfly(float* data) {
    #pragma unroll
...
template<int N>
__device__ inline void iwht_butterfly(float* data) {
    wht_butterfly<N>(data);
}
...
__global__ void tbq3_quantize_kernel(const half* __restrict__ src,
    ...
    for (int i = tid; i < TURBOQUANT_BLOCK_SIZE; i += blockDim.x) {
        ...
        // Pack 3-bit value
        // Simplified: store in shared memory first, then pack
        // (full packing would need careful bit manipulation)
        int byte_idx = i / 8;
        int bit_offset = (i % 8) * 3;
        // Use atomicOr for simplicity (can be optimized)
...
__global__ void tbqp3_quantize_kernel(const half* __restrict__ src,
    ...
    // QJL: 1-bit sign of residual after MSE quantization
...
// ============================================================================
// For TBQP types: apply QJL correction to attention scores during dot product
// This avoids full dequantization — just corrects the dot product

__global__ void qjl_attention_correct_kernel(
    const float* __restrict__ scores_in,
    float* __restrict__ scores_out,
    const uint8_t* __restrict__ qjl_signs,
    const half* __restrict__ q_wht,  // WHT-transformed query
    float d_qjl,
    int num_tokens,
    int head_dim
) {
    ...
    // Compute QJL correction: d_qjl * sum_i(q_wht[i] * sign_i)
    float qjl_dot = 0.0f;
...
// ============================================================================
// Compensates for quantization noise flattening softmax distribution
// alpha(N) = 1 + c * sqrt(ln(N))

__global__ void attention_sharpen_kernel(
    float* __restrict__ scores,
    int num_tokens,
    float sqnr,
    int is_tbpq  // TBQP needs stronger sharpening
) {
    int token = blockIdx.x * blockDim.x + threadIdx.x;
    if (token >= num_tokens) return;

    // Dynamic alpha based on context length
    float N = (float)num_tokens;
    float N0 = 2048.0f;  // reference context
    float c = is_tbpq ? 0.036f : 0.016f;  // TBQP vs TBQ coefficient
...
__global__ void fused_dequant_iwht_tbq3_kernel(
    ...
    // Apply IWHT
    iwht_butterfly<WHT_SIZE>(s_data);
    __syncthreads();
...
float turboquant_compression_ratio(turboquant_type_t type) {
    float f16_size = TURBOQUANT_BLOCK_SIZE * sizeof(half);  // 512 bytes
...
turboquant_type_t turboquant_auto_select(int head_dim, int quality_target) {
    // Auto-map by head_dim and quality
    switch (head_dim) {
        case 64:
...
void turboquant_dequantize_v(const void* src, half* dst, int num_blocks,
                             turboquant_type_t type, hipStream_t stream) {
    // V dequant is same as K for TBQ types, but for TBQP we use MSE only (no QJL)
...
void turboquant_encode_k(const half* src, void* dst, int num_blocks,
    ...
    switch (type) {
        ...
        case TURBOQUANT_TYPE_TBQ3_2:
            ...
            break;
        default:
            // Fallback: separate WHT + quantize
...
void turboquant_decode_k(const void* src, half* dst, int num_blocks,
    ...
    switch (type) {
        ...
        case TURBOQUANT_TYPE_TBQ3_2:
            ...
            break;
        default:
            // Fallback: separate dequant + IWHT
            turboquant_dequantize_k(src, dst, num_blocks, type, stream);
...
void turboquant_decode_v(const void* src, half* dst, int num_blocks,
                         int head_dim, turboquant_type_t type, hipStream_t stream) {
    // V decode: same as K for TBQ, MSE-only for TBQP
...
void turboquant_qjl_correct(float* scores, const uint8_t* qjl_signs,
    ...
    // This is called during attention — needs query WHT as well
    // For now, placeholder: full implementation needs attention integration
...
void turboquant_attention_sharpen(float* scores, int num_tokens,
    ...
    int is_tbpq = (sqnr < 20.0f) ? 1 : 0;  // TBQP has lower SQNR
