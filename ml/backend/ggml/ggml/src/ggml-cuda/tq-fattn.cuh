...
// Phase D — copy-and-patch of stock fattn-mma-f16 with TQ decode at the
// K load site. Stage 3: K-only presets, prefill (ncols1*ncols2 >= 8), Turing+.
// Returns true if Phase D handled the dispatch; false if the caller should
// fall back to the existing inline-decode path (Phase A/B/C).
