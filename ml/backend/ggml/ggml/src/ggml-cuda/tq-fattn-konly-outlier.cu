...
// K-only fused flash-attention with outlier-split (V_PACKED=false, HAS_OUTLIERS=true).
// Split into its own compilation unit so the 10-arch fatbinary from tq-fattn.cu
// does not exceed the gas single-object size limit (~2 GiB section size).
//
// Called by ggml_cuda_tq_flash_attn_ext when !v_packed && outlier_count > 0.
void ggml_cuda_tq_flash_attn_ext_konly_outlier(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    #define DISPATCH_KO_NCOLS_512(SOFTCAP) \
        if      (ncols == 1) { DISPATCH_KO(512, 1, SOFTCAP); } \
        else                 { DISPATCH_KO(512, 2, SOFTCAP); }
    ...
    if (D == 64) {
        ...
        if (logit_softcap == 0.0f) { DISPATCH_KO_NCOLS_512(false); }
        else                       { DISPATCH_KO_NCOLS_512(true);  }
    } else {
        if (logit_softcap == 0.0f) { DISPATCH_KO_NCOLS(128, false); }
        else                       { DISPATCH_KO_NCOLS(128, true);  }
    }

    #undef DISPATCH_KO_NCOLS_512
