...
void ggml_cuda_tq_wht(ggml_backend_cuda_context & ctx, struct ggml_tensor * dst) {
    ...
    // nb[] are in bytes; divide by sizeof(float) for element strides.
    // nb[0] is always sizeof(float) (contiguous in headDim).
    ...
    // Round down to power-of-2 (headDim is already power-of-2, block_size = min(headDim,128)).
    // For headDim < 128 the block_size equals headDim.
