...
__global__ void tq_dequant_kv_kernel_outlier(
    ...
    const int32_t *locs,
    int block_size  // passed explicitly to avoid %ntid.x on sm_120 (Pattern 10)
)
{
    ...
    // K phase: build outlier mask, decode K plane.
    {
        ...
        for (int w = threadIdx.x; w < mask_words; w += block_size) {
            s_mask[w] = 0u;
        }
        __syncthreads();
...
__global__ void tq_dequant_kv_kernel_outlier(
    ...
    uint16_t *, uint16_t *, int, int, int, const int32_t *, int) {}
#endif
...
__global__ void tq_dequant_k_outlier_wht_kernel(
    ...
    int block_size  // = k_wht_block; passed explicitly to avoid %ntid.x on sm_120
);
...
__global__ void tq_dequant_v_rotated_kernel(
    ...
    const int32_t * __restrict__ locs,
    int block_size  // = v_block; passed explicitly to avoid %ntid.x on sm_120
)
{
    ...
    // kForceFast=true: block is always launched with headDim threads (== n); avoids
    // sm_120 Pattern 3 miscompile of runtime nthreads>=n branch in this TU.
    apply_shs_wht<true, true>(s_rotV, rotation, headDim, threadIdx.x, block_size);
...
__global__ void tq_dequant_v_rotated_kernel(
    ...
    int, int, int, int, int, int, const int32_t * __restrict__, int) {}
#endif
...
)
{
    ...
    for (int elem = threadIdx.x; elem < headDim; elem += block_size) {
        ...
        int outliers_below = 0;
        const int full_words = elem >> 5;
        // Plain bounded loop — no #pragma unroll with runtime bound condition,
        // which miscompiles on sm_120 (Pattern 2: unrolled conditional increment
        // with runtime upper bound can be silently dropped).
        ...
        float val = (outlier_slot >= 0) ? out_val : reg_val;
        // Encode path: load K → subtract k_bias → WHT → subtract WHT-domain mean → quantize.
        // zeros_out is the WHT-domain mean, so decode must add it BEFORE WHT undo to
        // reconstruct the full WHT-domain vector, then undo the transform.
        if (zeros) {
            val += (outlier_slot >= 0) ? outZero : regZero;
        }
        s_wht[elem] = val;
    ...
    // kForceFast=true: block is always launched with headDim threads (== n); avoids
    // sm_120 Pattern 3 miscompile of runtime nthreads>=n branch in this TU.
    apply_shs_wht<false, true>(s_wht, signs, headDim, threadIdx.x, block_size);
...
__global__ void tq_dequant_k_outlier_wht_kernel(
    ...
    const float *, const float *, const int32_t *, int) {}
#endif
...
void ggml_cuda_tq_dequant_kv(ggml_backend_cuda_context & ctx, struct ggml_tensor * dst) {
    ...
    if (v_rotation != nullptr && k_has_outliers) {
        ...
        // V WHT-undo uses apply_shs_wht<kForceFast=true>, which requires one
        // thread per element (nthreads == headDim). Mirror the K kernel's
        // sizing (headDim threads, capped at 256). Capping at 128 silently
        // broke D=256: only the first half of the butterfly ran, corrupting V
        // for gemma3 K+V (PPL 63M). D>=512 K+V routes to Path 2, not here.
        const int    v_block  = headDim <= 256 ? headDim : 256;
    ...
    // RDNA3 doesn't (handled by the GGML_USE_HIP override below).
    bool use_combined = k_has_outliers;
#ifdef GGML_USE_HIP
    use_combined = false;
#endif
    if (use_combined) {
