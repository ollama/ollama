...
static __host__ inline TQFattnCtx tq_fattn_extract(const ggml_tensor * dst, int D) {
    ...
    const ggml_tensor * qjl_proj_t    = dst->src[11];
    ...
    memcpy(&c.qjl_rows,             (const int32_t *)dst->op_params + 5, sizeof(int32_t));
    ...
    c.qjl_packed_ptr  = qjl_packed_t ? (const uint8_t *)qjl_packed_t->data : nullptr;
    c.qjl_norm_ptr    = qjl_norm_t   ? (const float   *)qjl_norm_t->data   : nullptr;
    c.qjl_projection_ptr = qjl_proj_t ? (const float  *)qjl_proj_t->data   : nullptr;
    const int qjl_packed_raw = (c.qjl_rows + 7) / 8;
    c.qjl_packedBytes = (qjl_packed_raw + 3) & ~3;
...
#define TQ_DISPATCH_DIM(CTX, DIM, NCOLS, SOFTCAP) \
    if ((CTX).v_packed && (CTX).has_outliers) { \
        tq_fattn_vec_launch<DIM, NCOLS, SOFTCAP, true, true>(ctx, dst, (CTX).scale, (CTX).logit_softcap, \
            ...
            (CTX).qjl_packed_ptr, (CTX).qjl_norm_ptr, (CTX).qjl_projection_ptr, (CTX).qjl_rows, (CTX).qjl_packedBytes, \
