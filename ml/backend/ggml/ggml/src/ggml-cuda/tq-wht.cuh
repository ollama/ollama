...
//
// Template parameters:
//   kTrailingSync: when false, the final __syncthreads() is omitted. Safe only
//     when every thread reads only its own position(s) of s_x after return.
//     Encode callers must leave this true so subsequent cross-thread reads are safe.
//   kForceFast: when true, unconditionally take the fast path (one thread per
//     element). Caller must guarantee nthreads == n. Avoids sm_120 Pattern 3
//     miscompile of the runtime `nthreads >= n` branch. The dequant TU requires
//     this unconditionally; encode TUs use it conditionally when block_size==headDim
//     (i.e. D<=128), falling back to kForceFast=false for D=256 where block_size<n.
template<bool kTrailingSync = true, bool kForceFast = false>
    ...
    int n, int tid, int nthreads)
{
    ...
    if constexpr (kForceFast) {
        // Fast path: one thread per element (caller guarantees nthreads == n).
        // Compile-time dispatch avoids sm_120 runtime branch miscompile (Pattern 3).
        float val = s_x[tid];
