...
// are regression gates for the 5-bit outlier decode path in the Metal fused FA kernels.
//
// Root cause (fixed in 9c68aa6d / 5125528a): kernel_tq_fattn_vec_f16_outlier and
// related variants only had explicit decode branches for outlierBits==3 and
// outlierBits==4. The else fallback read only 2 bits per outlier index. Production
// tq4/tq4k use outlierBits=5 (32-entry codebook), so the fallback was decoding from
// codebook[0..3] instead of codebook[0..31] — always picking the smallest-magnitude
// outlier entries and producing sign flips and 5× magnitude errors on the outlier
// channels that matter most.
//
// These tests cover:
//   - Symmetric 4-bit path: EncodeKeyPerHeadOutlier → fused FA, compared to CPU decode
//   - Asymmetric 4-bit path: same as TestTQFusedFlashAttentionOutlierAsymmetric but
//     with bits=4, outlierBits=5 — the production tq4k configuration.
...
func TestTQFusedFlashAttentionOutlier4bit(t *testing.T) {
	...
	const (
		headDim      = 128
		nKVHeads     = 8
		nCells       = 8 // small to hit the prefill-scale bug (stride error per cell)
		bits         = 4
		outlierBits  = 5
		outlierCount = 32
	)
