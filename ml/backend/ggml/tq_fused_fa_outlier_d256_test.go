...
// Correctness tests for the D=256 outlier fused FA kernel on CUDA and Metal
// (ggml_cuda_tq_flash_attn_ext_d256 / kernel_tq_fattn_vec_f16_outlier_d256)
...
//
// D=256 covers Gemma3/Gemma4 local-attention layers (head_dim=256). D=128 covers llama3.1/qwen2.5.
// fusedKernelSupports() returns true for D=256 on CUDA/ROCm and Metal.
	...
	for _, cfg := range d256Configs {
		...
		t.Run(fmt.Sprintf("bits%d_outlierBits%d", cfg.bits, cfg.outlierBits), func(t *testing.T) {
			...
			if !mgr.fusedKernelSupports() {
				t.Skipf("fused kernel not supported for headDim=%d bits=%d (unsupported bits config)", headDim, cfg.bits)
			}
...
func TestTQFusedFlashAttentionOutlierD256Prefill(t *testing.T) {
	...
	if !mgr.fusedKernelSupports() {
		t.Skipf("fused kernel not supported for headDim=%d bits=%d (unsupported bits config)", headDim, bits)
	}
