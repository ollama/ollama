...
type splitGGUFCollector struct {
	slots  []splitGGUFSlot
	groups map[splitGGUFKey][]*layerGGML
}

func newSplitGGUFCollector() *splitGGUFCollector {
	return &splitGGUFCollector{
		groups: make(map[splitGGUFKey][]*layerGGML),
	}
}
...
type splitGGUFTensorReader struct {
	ctx context.Context //nolint:containedctx // io.WriterTo has no context parameter; this reader lives only for one merge.
...
func (r splitGGUFTensorReader) WriteTo(w io.Writer) (int64, error) {
	...
	return io.CopyN(w, readerWithContext(r.ctx, io.NewSectionReader(f, r.offset, r.size)), r.size)
}
...
func splitGGUFGroupKey(layer *layerGGML) (splitGGUFKey, bool, error) {
	...
	if !ok {
		return splitGGUFKey{}, false, fmt.Errorf("split GGUF %q must use llama.cpp split filename pattern", layer.From)
	}
	if uint64(nameCount) != count {
		return splitGGUFKey{}, false, fmt.Errorf("split GGUF %q filename count %d does not match metadata count %d", layer.From, nameCount, count)
	}
	if uint64(index) >= count {
		return splitGGUFKey{}, false, fmt.Errorf("split GGUF %q filename index %d exceeds metadata count %d", layer.From, index, count)
	}
	splitNo, ok := splitGGUFUint64(layer.GGML.KV(), "split.no")
	if !ok {
		return splitGGUFKey{}, false, fmt.Errorf("split GGUF %q is missing split.no metadata", layer.From)
	}
	if splitNo != uint64(index) {
		return splitGGUFKey{}, false, fmt.Errorf("split GGUF %q filename index %d does not match metadata index %d", layer.From, index, splitNo)
	}

	return splitGGUFKey{
		mediaType: layer.MediaType,
		dir:       path.Dir(layer.From),
		prefix:    prefix,
		count:     nameCount,
	}, true, nil
}
...
func mergeSplitGGUFToLayer(ctx context.Context, layer *layerGGML) (*layerGGML, error) {
	...
	tensors, err := splitGGUFTensors(ctx, layer)
	if err != nil {
		return nil, invalidSplitGGUF(err)
	}
	...
	merged, err := manifest.NewLayer(readerWithContext(ctx, temp), layer.MediaType)
	if err != nil {
		return nil, err
	}
...
func splitGGUFTensors(ctx context.Context, layer *layerGGML) ([]*ggml.Tensor, error) {
