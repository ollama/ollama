...
func chatPrompt(ctx context.Context, m *Model, tokenize tokenizeFunc, opts *api.Options, msgs []api.Message, tools []api.Tool) (prompt string, images []llm.ImageData, audioUrls, videoUrls []string, _ error) {
	...
	for i := n; i >= 0; i-- {
		if isMllama && len(msgs[i].Images) > 1 {
			return "", nil, nil, nil, errTooManyImages
		}
		...
		for _, i := range msg.Images {
			...
			if isMllama {
				if len(m.ProjectorPaths) == 0 {
					...
					if !ok {
						return "", nil, nil, nil, fmt.Errorf("missing aspect ratio for image")
					}
	...
	return b.String(), images, audioUrls, videoUrls, nil
}
