...
func (s *Scheduler) load(req *LlmRequest, f *ggml.GGML, systemInfo ml.SystemInfo, gpus []ml.DeviceInfo, requireFull bool) bool {
	...
	for _, gpu := range gpus {
		CappedFreeMemory := min(gpu.FreeMemory, envconfig.MaxVRAM())

		available := CappedFreeMemory - envconfig.GpuOverhead() - gpu.MinimumMemory()
		if CappedFreeMemory < envconfig.GpuOverhead()+gpu.MinimumMemory() {
			available = 0
		}
		slog.Info("gpu memory", "id", gpu.ID, "library", gpu.Library,
			"available", format.HumanBytes2(available),
			"free", format.HumanBytes2(CappedFreeMemory),
