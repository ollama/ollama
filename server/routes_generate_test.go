...
func newMockServer(mock *mockRunner) func(discover.GpuInfoList, string, *modelfile.GGML, []string, []string, api.Options, int) (runners.LLMServer, error) {
	return func(gpus discover.GpuInfoList, model string, ggml *modelfile.GGML, projectors, system []string, opts api.Options, numParallel int) (runners.LLMServer, error) {
		return mock, nil
	}
}
...
`, createBinFile(t, modelfile.KV{
	...
	t.Run("missing capabilities chat", func(t *testing.T) {
		w := createRequest(t, s.CreateHandler, api.CreateRequest{
			Model: "bert",
			Modelfile: fmt.Sprintf("FROM %s", createBinFile(t, modelfile.KV{
