...
func TestListIncludesManifestListChildrenAsSeparateRows(t *testing.T) {
	...
	parentModified := parentInfo.ModTime()
...
func TestBuildModelListSummaryRejectsInvalidRunner(t *testing.T) {
	...
	if err := manifest.WriteManifestWithMetadata(model.ParseName("test-invalid"), cfg, []manifest.Layer{layer}, "bad", manifest.FormatGGUF); err != nil {
		t.Fatal(err)
	}

	mf, err := manifest.ParseNamedManifest(model.ParseName("test-invalid"))
	if err != nil {
		t.Fatal(err)
	}
	_, err = buildModelListSummary(model.ParseName("test-invalid"), mf, mf.Digest(), mf.Size())
	if err == nil || err.Error() != `unknown runner "bad"` {
		t.Fatalf("error = %v, want unknown runner", err)
	}
}
