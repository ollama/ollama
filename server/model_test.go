...
func TestParseFromFileFromLayer(t *testing.T) {
	...
	defer file.Close()
	if err := modelfile.WriteGGUF(file, modelfile.KV{"general.architecture": "gemma"}, []modelfile.Tensor{}); err != nil {
...
func TestParseLayerFromCopy(t *testing.T) {
	...
	for range 5 {
		if err := modelfile.WriteGGUF(file2, modelfile.KV{"general.architecture": "gemma"}, []modelfile.Tensor{}); err != nil {
			t.Fatalf("failed to write gguf: %v", err)
		}
	}
