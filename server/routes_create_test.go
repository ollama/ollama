...
func TestCreateModelRetainsSplitGGUF(t *testing.T) {
	...
	// A split GGUF cannot be loaded from its raw blob path: llama.cpp derives
	// the remaining shards from the first shard's filename, so ModelPath is
	// rewritten to a correctly named link under blobs/splits/ rather than the
	// bare content-addressed blob.
	if got := filepath.Base(created.ModelPath); got != "model-00001-of-00002.gguf" {
		t.Fatalf("model path = %q, want a first-shard split name", created.ModelPath)
	}
	...
	firstLinkInfo, err := os.Stat(created.ModelPath)
	if err != nil {
		t.Fatal(err)
	}
	firstBlobInfo, err := os.Stat(firstPath)
	if err != nil {
		t.Fatal(err)
	}
	if !os.SameFile(firstLinkInfo, firstBlobInfo) {
		t.Fatalf("model path %q does not link to the first shard's blob", created.ModelPath)
	}
	// llama.cpp derives the remaining shards from the first shard's link by
	// rewriting the index itself, so ModelShardPaths keeps the plain blob
	// path here rather than a split-named link.
...
func TestShardedGGUF(t *testing.T) {
	...
	_, fullDigest := createBinFile(t, map[string]any{}, nil)
	_, splitDigest1 := createBinFile(t, map[string]any{
		"split.no":    uint16(0),
		"split.count": uint16(3),
	}, nil)
	_, splitDigest2 := createBinFile(t, map[string]any{
		"split.no":    uint16(1),
		"split.count": uint16(3),
	}, nil)
	_, splitDigest3 := createBinFile(t, map[string]any{
		"split.no":    uint16(2),
		"split.count": uint16(3),
	}, nil)
	_, splitDigest4 := createBinFile(t, map[string]any{
		"split.no":    uint16(0),
		"split.count": uint16(4),
	}, nil)
	_, splitDigest5 := createBinFile(t, map[string]any{
	...
	t.Run("mixed full and split gguf", func(t *testing.T) {
		...
		// Files are processed in sorted filename order, and a split GGUF
		// group collapses to a single manifest layer at the position of its
		// first-encountered shard. "test-00001..." sorts before "test1.gguf"
		// (ASCII '-' < '1'), so the split group comes first.
		correctOrder := []string{
			splitDigest1, splitDigest2, splitDigest3, fullDigest,
		}
