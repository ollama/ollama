...
func (s *Server) GenerateRoutes(rc *ollama.Registry) (http.Handler, error) {
	...
	// Model Management (Phase 8)
	r.GET("/api/models/available", s.ListAvailableModelsHandler)
	r.GET("/api/models/local", s.ListLocalModelsHandler)
	r.GET("/api/models/:id", s.GetModelInfoHandler)
	r.POST("/api/models/download", s.DownloadModelHandler)
	r.GET("/api/models/:id/progress", s.GetDownloadProgressHandler)
	r.DELETE("/api/models/:id/download", s.CancelDownloadHandler)
	r.DELETE("/api/models/:id", s.DeleteModelHandler)
	r.GET("/api/models/compare", s.CompareModelsHandler)
	r.GET("/api/models/storage", s.GetStorageStatsHandler)

	// Performance Monitoring (Phase 9)
	r.GET("/api/metrics/performance", s.GetPerformanceMetricsHandler)
	r.GET("/api/metrics/provider/:provider", s.GetProviderMetricsHandler)
	r.GET("/api/metrics/system", s.GetSystemMetricsHandler)
	r.GET("/api/metrics/providers/top", s.GetTopProvidersHandler)
	r.GET("/api/metrics/cost", s.GetCostBreakdownHandler)
	r.GET("/api/metrics/alerts", s.GetAlertsHandler)
	r.POST("/api/metrics/reset", s.ResetMetricsHandler)
	r.POST("/api/metrics/custom", s.RecordCustomMetricHandler)
