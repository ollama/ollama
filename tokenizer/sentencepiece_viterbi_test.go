...
// TestSentencePieceByteFallbackPreservesLaterTokens verifies that a token
// appearing AFTER an OOV rune is still emitted. Before byte-fallback edges
// were added to the DP, the DP couldn't reach positions past an OOV rune,
// the traceback fell into l=0 recovery for every subsequent position, and
// regular vocab tokens (which have no <0xXX> entries) were silently dropped.
func TestSentencePieceByteFallbackPreservesLaterTokens(t *testing.T) {
	spm := makeUnigramVocab([]struct {
		value string
		score float32
		typ   int32
		...
		{"hello", -1.0, TOKEN_TYPE_NORMAL}, // 0
		{"world", -1.0, TOKEN_TYPE_NORMAL}, // 1
		{"<0xC3>", -10.0, TOKEN_TYPE_BYTE}, // 2: first byte of é
		{"<0xA9>", -10.0, TOKEN_TYPE_BYTE}, // 3: second byte of é
		{"<0x21>", -10.0, TOKEN_TYPE_BYTE}, // 4: '!'
	})

	// "hello" + "é" (OOV, byte fallback) + "world": "world" must still appear.
	ids, err := spm.Encode("helloéworld", false)
	if err != nil {
		t.Fatal(err)
	}
	want := []int32{0, 2, 3, 1} // hello, <0xC3>, <0xA9>, world
	if !slices.Equal(ids, want) {
		pieces := make([]string, len(ids))
		for i, id := range ids {
			pieces[i] = spm.vocab.Values[id]
		}
		t.Errorf("got %v (%v), want %v (hello, <0xC3>, <0xA9>, world)", ids, pieces, want)
	}
}
