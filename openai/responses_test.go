...
func TestResponsesStreamConverter_ProcessError(t *testing.T) {
	t.Run("mid stream", func(t *testing.T) {
		...
		// The partial content already went out as deltas and in the closed
		// items' own output_item.done events, so the failed response itself
		// carries no output.
	...
	t.Run("closes an open message item as incomplete", func(t *testing.T) {
		converter := NewResponsesStreamConverter("resp_123", "msg_456", "gpt-oss:20b", ResponsesRequest{})
		...
		// The message item is closed so the client is not left holding an
		// in_progress item, but as incomplete rather than claiming it finished.
		events := converter.ProcessError("error parsing tool call")
		want := []string{
			"response.output_text.done",
			"response.content_part.done",
		...
		for i, event := range events {
			if event.Event != want[i] {
				t.Errorf("events[%d] = %q, want %q", i, event.Event, want[i])
			}
		}

		item := events[2].Data.(map[string]any)["item"].(map[string]any)
		if item["status"] != "incomplete" {
			t.Errorf("output_item.done status = %q, want %q", item["status"], "incomplete")
		}
		if text := item["content"].([]map[string]any)[0]["text"]; text != "partial answer" {
			t.Errorf("output_item.done text = %q, want %q", text, "partial answer")
		}
	})
