...
func glm47FlashModelKV(src *SourceModel, mla glm47FlashMLA) outKV {
	modelKV := outKV{}
	for _, keyValue := range src.GGUF.KeyValues() {
		...
		case strings.HasPrefix(key, "glm4moelite."):
			modelKV[strings.Replace(key, "glm4moelite.", "deepseek2.", 1)] = normalizeGGUFValue(keyValue.Any())
...
func glm47FlashSetExtraEOGFromEOSIDs(kv outKV) {
...
func glm47FlashSplitKVB(t *sourceTensor, mla glm47FlashMLA) ([]*outTensor, error) {
	if len(t.shape) != 2 {
...
func glm47FlashRepackKVB(mla glm47FlashMLA, extractK bool, kvFirst bool, numHeads int) repacker {
	return func(name string, data []float32, shape []uint64) ([]float32, error) {
		if kvFirst {
			var err error
			if data, _, err = permuteTensor(data, tensorDims(shape), 1, 0); err != nil {
				return nil, err
			}
		}

		headDims := []int{numHeads, mla.qkNope + mla.vHeadDim, mla.kvLoraRank}
		if err := checkTensorDims(data, headDims); err != nil {
			return nil, fmt.Errorf("glm-4.7-flash migration cannot reshape %s from %v to %v: %w", name, shape, headDims, err)
		}

		if !extractK {
			return sliceTensorDim(data, headDims, 1, mla.qkNope, mla.qkNope+mla.vHeadDim)
		}

		k, err := sliceTensorDim(data, headDims, 1, 0, mla.qkNope)
		if err != nil {
			return nil, err
		}
		headDims[1] = mla.qkNope
		k, _, err = permuteTensor(k, headDims, 0, 2, 1)
		return k, err
	}
}
