...
func (qwen3VLMigrator) Migrate(src *SourceModel) (*Result, error) {
	...
	if sections := qwen3VLMropeSections(src); len(sections) > 0 {
		modelKV[arch+".rope.dimension_sections"] = sections
	}
	modelKV[arch+".n_deepstack_layers"] = uint32(len(deepstackIndexes))
...
func qwen3VLSplitLegacyPatchTensor(t *sourceTensor, numChannels uint64) ([]*outTensor, error) {
	if numChannels == 0 {
...
func qwen3VLConcatQKVWeights(name string, tensors ...*sourceTensor) (*outTensor, error) {
	if len(tensors) == 0 {
