...
func copyQwen35MTPTensor(name string, tensor *sourceTensor) *outTensor {
	writer := tensor.Clone()
...
func qwen35MergedMTPExpertTensors(groups map[string]map[int]*sourceTensor) ([]*outTensor, error) {
	var out []*outTensor
...
func qwen35ProjectorKV(src *SourceModel) outKV {
