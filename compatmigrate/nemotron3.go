...
func nemotron3ModelKV(src *SourceModel) outKV {
	arch := nemotron3TextArchitecture(src)
	out := outKV{"general.architecture": arch}
...
func nemotron3ProjectorKV(src *SourceModel) outKV {
	...
	for _, key := range []string{
		...
		if value := src.GGUF.KeyValue(key); value.Valid() {
			out["clip."+key] = normalizeGGUFValue(value.Any())
		}
	}
	if mean := src.GGUF.KeyValue("vision.image_mean"); mean.Valid() {
		out["clip.vision.image_mean"] = normalizeGGUFValue(mean.Any())
	}
	if std := src.GGUF.KeyValue("vision.image_std"); std.Valid() {
		out["clip.vision.image_std"] = normalizeGGUFValue(std.Any())
	}
...
func nemotron3ProjectorTensor(src *SourceModel, tensor *sourceTensor) ([]*outTensor, error) {
	name := tensor.name
