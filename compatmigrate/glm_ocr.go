...
func glmOCRModelKV(src *SourceModel) outKV {
	modelKV := outKV{}
	for _, keyValue := range src.GGUF.KeyValues() {
		...
		case key == "glm4.rope.mrope_section":
			modelKV["glm4.rope.dimension_sections"] = padInt32Sections(normalizeGGUFValue(keyValue.Any()))
		...
		case strings.HasPrefix(key, "glmocr."):
			modelKV[strings.Replace(key, "glmocr.", "glm4.", 1)] = normalizeGGUFValue(keyValue.Any())
		case key == "rope.mrope_section":
			modelKV["glm4.rope.dimension_sections"] = padInt32Sections(normalizeGGUFValue(keyValue.Any()))
		case glmOCRTextKVKey(key):
			modelKV["glm4."+key] = normalizeGGUFValue(keyValue.Any())
	...
	if _, ok := modelKV["glm4.rope.dimension_sections"]; !ok {
		if sections := src.GGUF.KeyValue("rope.dimension_sections"); sections.Valid() {
			modelKV["glm4.rope.dimension_sections"] = padInt32Sections(normalizeGGUFValue(sections.Any()))
		}
	}
	return modelKV
}
...
func glmOCRProjectorKV(src *SourceModel) outKV {
	projectorKV := outKV{
