...
func copyTensor(name string, t *sourceTensor) *outTensor {
	writer := t.Clone()
...
func normalizeGGUFValue(v any) any {
	...
	case reflect.Slice:
		...
		case reflect.Float32, reflect.Float64:
			// Model metadata floats fit comfortably in float32; the GGUF
			// writer has no float64 array support and llama.cpp expects f32
			// arrays for the keys the migrators carry.
			out := make([]float32, rv.Len())
			for i := range out {
				out[i] = float32(rv.Index(i).Convert(reflect.TypeFor[float64]()).Float())
			}
			return out
		...
		case reflect.Int, reflect.Int64:
			// Preserve 64-bit values (e.g. token ids) rather than silently
			// truncating them to 32 bits.
...
// requiredBytesFromSource estimates the disk space a conversion needs: the
// converted model is roughly the size of the source model layer, padded by
// the migration margin for metadata growth and tensor dtype promotions, plus
// fixed headroom. Projector layers are small relative to the model, so they
// are covered by the margin and headroom rather than summed.
func requiredBytesFromSource(src *SourceModel) uint64 {
	...
	return sourceBytes + sourceBytes/compatMigrationMarginDenom + compatMigrationHeadroom
}
