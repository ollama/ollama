...
func TestEnsureLocalCompatibilityMigrationSkipsAdapterModels(t *testing.T) {
	...
	writeSourceManifest(t, source, sourceManifestInput{
		...
		modelTensors: []*outTensor{
			fixtureTensor("token_embd.weight", gguf.TensorTypeF32, []uint64{2, 2}),
		},
...
func TestEnsureLocalCompatibilityMigrationGemma4(t *testing.T) {
	...
	writeSourceManifest(t, source, sourceManifestInput{
		...
		modelTensors: []*outTensor{
			fixtureTensor("token_embd.weight", gguf.TensorTypeF16, []uint64{2, 32}),
			fixtureTensor("blk.0.attn_q.weight", gguf.TensorTypeF16, []uint64{32, 32}),
			fixtureTensor("blk.0.ffn_gate_exps.weight", gguf.TensorTypeF16, []uint64{32, 16, 2}),
			fixtureTensor("blk.0.ffn_up_exps.weight", gguf.TensorTypeF16, []uint64{32, 16, 2}),
			fixtureTensor("blk.0.ffn_gate_inp.per_expert_scale", gguf.TensorTypeF32, []uint64{2}),
			fixtureTensor("v.patch_embd.weight", gguf.TensorTypeF16, []uint64{16, 16, 3, 24}),
			fixtureTensor("mm.input_projection.weight", gguf.TensorTypeF16, []uint64{24, 32}),
			fixtureTensor("a.blk.0.attn_q.weight", gguf.TensorTypeBF16, []uint64{16, 16}),
			fixtureTensor("a.blk.0.linear_pos.weight", gguf.TensorTypeBF16, []uint64{16, 16}),
			fixtureTensor("a.blk.0.ln1.weight", gguf.TensorTypeF32, []uint64{16}),
			fixtureTensor("a.blk.0.ln2.weight", gguf.TensorTypeF32, []uint64{16}),
			fixtureTensor("a.blk.0.layer_pre_norm.weight", gguf.TensorTypeF32, []uint64{16}),
			fixtureTensor("a.pre_encode.out.weight", gguf.TensorTypeF16, []uint64{16, 16}),
			fixtureTensor("mm.a.input_projection.weight", gguf.TensorTypeF16, []uint64{16, 32}),
			fixtureTensor("mm.a.fc.weight", gguf.TensorTypeF16, []uint64{16, 32}),
		},
...
func TestEnsureLocalCompatibilityMigrationLaguna(t *testing.T) {
	...
	writeSourceManifest(t, source, sourceManifestInput{
		...
		modelTensors: []*outTensor{
			fixtureTensor("token_embd.weight", gguf.TensorTypeF16, []uint64{2, 32}),
			fixtureTensor("blk.0.attn_q.weight", gguf.TensorTypeF16, []uint64{32, 32}),
			fixtureTensor("blk.0.attn_g.weight", gguf.TensorTypeF16, []uint64{32, 32}),
			fixtureTensor("blk.1.attn_g.weight", gguf.TensorTypeF16, []uint64{32, 32}),
		},
...
func TestQwen3VLSplitLegacyPatchTensorOrder(t *testing.T) {
	...
	raw := make([]byte, tensorBytes(gguf.TensorTypeF16, shape))
...
func TestNeedsMigrationAlreadyConverted(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			tensors: []*outTensor{
				fixtureTensor("dense.0.bias", gguf.TensorTypeF16, []uint64{8}),
			},
			want: false,
		},
		{
			...
			tensors: []*outTensor{
				fixtureTensor("dense.0.weight", gguf.TensorTypeF16, []uint64{8, 8}),
			},
			want: true,
		},
		{
			...
			tensors: []*outTensor{
				fixtureTensor("token_embd.weight", gguf.TensorTypeF16, []uint64{2, 8}),
			},
			want: false,
		},
		{
		...
		},
		{
			...
			tensors: []*outTensor{
				fixtureTensor("blk.0.attn_gate.weight", gguf.TensorTypeF16, []uint64{8, 8}),
			},
			want: false,
		},
		{
			...
			tensors: []*outTensor{
				fixtureTensor("blk.0.attn_g.weight", gguf.TensorTypeF16, []uint64{8, 8}),
			},
			want: true,
		},
		{
		...
		},
		{
			...
			tensors: []*outTensor{
				fixtureTensor("blk.0.ssm_dt.bias", gguf.TensorTypeF32, []uint64{8}),
			},
			want: false,
		},
		{
			...
			tensors: []*outTensor{
				fixtureTensor("blk.0.ssm_dt", gguf.TensorTypeF32, []uint64{8}),
			},
			want: true,
		},
		{
		...
		},
		{
			...
			projectorTensors: []*outTensor{
				fixtureTensor("mm.0.weight", gguf.TensorTypeF16, []uint64{2, 2}),
			},
			want: true,
		},
		{
...
func TestGPTOSSExpertFeedForwardLengthFromTensor(t *testing.T) {
	src := fixtureSourceModel(t, outKV{
...
func fixtureSourceModel(t *testing.T, kv outKV, tensors []*outTensor) *SourceModel {
	t.Helper()
...
func (testCompatMigrator) Migrate(src *SourceModel) (*Result, error) {
	...
	out := make([]*outTensor, 0, len(tensors))
...
func writeFixtureGGUFLayer(t *testing.T, kv outKV, tensors []*outTensor) manifest.Layer {
	t.Helper()
...
func fixtureTensor(name string, kind gguf.TensorType, shape []uint64) *outTensor {
	return &outTensor{
		Name:     name,
		Kind:     uint32(kind),
		Shape:    shape,
...
func tensorBytes(kind gguf.TensorType, shape []uint64) int {
	var values uint64 = 1
	for _, dim := range shape {
		values *= dim
	}
	return int(float64(values) * kind.NumBytes())
}

func writeTensorF32(t *testing.T, tensor *outTensor) []float32 {
	t.Helper()
...
func writeTensorF16(t *testing.T, tensor *outTensor) []float32 {
	t.Helper()
...
func TestEnsureLocalCompatibilityMigrationRepairsDanglingV2Entry(t *testing.T) {
	...
	// Simulate a store damaged by a pre-manifest-list daemon: the v2 entry
	// dangles because the old garbage collector removed the manifest-list
	// document blob, while the legacy source manifest survives.
	sourceData, err := manifest.ReadManifestData(source)
	if err != nil {
		t.Fatalf("ReadManifestData(source) error = %v", err)
	}
	if err := manifest.WriteLegacyManifestData(source, sourceData); err != nil {
		t.Fatalf("WriteLegacyManifestData(source) error = %v", err)
	}

	v2Path, err := manifest.V2PathForName(source)
	if err != nil {
		t.Fatalf("V2PathForName(source) error = %v", err)
	}
	if err := os.MkdirAll(filepath.Dir(v2Path), 0o755); err != nil {
		t.Fatalf("mkdir v2 dir: %v", err)
	}
	if err := os.RemoveAll(v2Path); err != nil {
		t.Fatalf("remove v2 entry: %v", err)
	}
	missing := filepath.Join("..", "..", "blobs", "sha256-"+strings.Repeat("0", 64))
	if err := os.Symlink(missing, v2Path); err != nil {
	...
	raw, err := manifest.ReadManifestData(source)
	if err != nil {
		t.Fatalf("ReadManifestData after repair error = %v", err)
	}
	var parent manifest.Manifest
	if err := json.Unmarshal(raw, &parent); err != nil {
		t.Fatalf("unmarshal repaired parent manifest: %v", err)
	}
	if parent.MediaType != manifest.MediaTypeManifestList {
		t.Fatalf("repaired manifest mediaType = %q, want manifest list", parent.MediaType)
	}
	if _, err := manifest.ParseNamedManifestForRunner(source, manifest.RunnerLlamaCPP); err != nil {
