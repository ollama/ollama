...
func gemma3ModelKV(src *SourceModel, vocabSize int) outKV {
	modelKV := outKV{}
	var globalRope any
	var localRope any
	...
	// Fallback defaults below mirror the upstream google/gemma-3 configuration for legacy files that omitted them.
...
func gemma3ProjectorKV(src *SourceModel) outKV {
	// image_size 896, patch_size 14, and 0.5 mean/std are the SigLIP vision defaults for Gemma 3.
...
// gemma3VocabSize infers the vocab size from the trailing dim of token_embd.weight
// because legacy files' tokenizer arrays can be longer than the actual vocab.
...
func gemma3ProjectorTensorName(name string) string {
	...
	// The fc1->ffn_down / fc2->ffn_up mapping looks reversed but intentionally mirrors llama.cpp's clip naming for the gemma3 projector.
