...
func deepseekOCRModelKV(src *SourceModel) outKV {
	modelKV := outKV{}
	for _, keyValue := range src.GGUF.KeyValues() {
		...
		case strings.HasPrefix(key, "deepseekocr."):
			modelKV[strings.Replace(key, "deepseekocr.", "deepseek2-ocr.", 1)] = normalizeGGUFValue(keyValue.Any())
		case strings.HasPrefix(key, "deepseek2-ocr."):
			modelKV[key] = normalizeGGUFValue(keyValue.Any())
		case deepseekOCRTextKVKey(key):
			modelKV["deepseek2-ocr."+key] = normalizeGGUFValue(keyValue.Any())
...
func deepseekOCRProjectorKV(src *SourceModel) outKV {
	return outKV{
...
func deepseekOCRProjectorTensorName(name string) string {
	replacer := strings.NewReplacer(
		...
		"mm.view_seperator", "v.view_seperator", //nolint:misspell // published DeepSeek OCR tensor spelling
