...
func mistralPixtralModelKV(src *SourceModel) outKV {
	modelKV := outKV{}
...
func mistralPixtralProjectorKV(src *SourceModel) outKV {
	eps := float32(src.GGUF.KeyValue("vision.attention.layer_norm_epsilon").Float())
	if eps == 0 {
		eps = 1e-5 // Mirrors the Pixtral config's default vision attention layer-norm epsilon.
	}
...
func copyMistralPixtralProjectorTensor(t *sourceTensor, visionHeads uint64) *outTensor {
