...
func splitSourceTensorDim(t *sourceTensor, dim int, names ...string) ([]*outTensor, error) {
	...
	out := make([]*outTensor, 0, len(names))
	offset := 0
	for _, name := range names {
		...
		writer.SetRepacker(func(_ string, data []float32, sourceShape []uint64) ([]float32, error) {
			return sliceTensorDim(data, tensorDims(sourceShape), dim, start, end)
		})
...
func concatSourceTensorsDim(dim int, name string, tensors ...*sourceTensor) (*outTensor, error) {
	if len(tensors) == 0 {
...
func concatSourceTensorsSlowDim(dim int, name string, tensors ...*sourceTensor) (*outTensor, error) {
	if len(tensors) == 0 {
		return nil, fmt.Errorf("concat tensor %s: no source tensors", name)
	}
	if dim < 0 || dim >= len(tensors[0].shape) {
...
func concatSourceTensorsDimKind(dim int, name string, kind gguf.TensorType, tensors ...*sourceTensor) (*outTensor, error) {
	if len(tensors) == 0 {
		return nil, fmt.Errorf("concat tensor %s: no source tensors", name)
	}
	if dim < 0 || dim >= len(tensors[0].shape) {
...
func stackSourceTensors(name string, tensors ...*sourceTensor) (*outTensor, error) {
	if len(tensors) == 0 {
	...
	if len(shape) >= 3 && shape[2] == 1 {
		// Some converters emit per-item tensors with a singleton third dim;
		// reuse it as the stack dimension instead of appending a fourth.
...
func (c sourceTensorConcat) WriteTo(w io.Writer) (int64, error) {
	parts := make([]tensorPart, 0, len(c.tensors))
	for _, source := range c.tensors {
		data, err := readSourceTensorFloatData(source)
		if err != nil {
			return 0, err
		}
		parts = append(parts, tensorPart{data: data, dims: tensorDims(source.shape)})
	}

	data, _, err := concatTensorDim(c.dim, parts...)
	if err != nil {
		return 0, err
	}
...
func copyTensorPrefix(name string, t *sourceTensor, shape []uint64) *outTensor {
	writer := t.Clone()
	writer.shape = slices.Clone(shape)
