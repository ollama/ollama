...
func (qwen25VLMigrator) Migrate(src *SourceModel) (*Result, error) {
	...
	var projectorTensors []*outTensor
	...
	for _, keyValue := range src.GGUF.KeyValues() {
		...
		case strings.HasPrefix(key, "qwen25vl."):
			modelKV[strings.Replace(key, "qwen25vl.", "qwen2vl.", 1)] = normalizeGGUFValue(keyValue.Any())
	...
	if sections := qwen25VLMropeSections(src); len(sections) > 0 {
		modelKV["qwen2vl.rope.dimension_sections"] = sections
	}

	hidden := uint32(src.GGUF.KeyValue("embedding_length").Uint())
	visionHidden := uint32(src.GGUF.KeyValue("vision.embedding_length").Uint())
	nWAPattern := uint32(8) // Qwen2.5-VL window-attention pattern: full attention every 8th layer.
	...
	if fullatt := src.GGUF.KeyValue("vision.fullatt_block_indexes"); fullatt.Valid() {
		projectorKV["clip.vision.fullatt_block_indexes"] = normalizeGGUFValue(fullatt.Any())
	} else {
	...
	if mean := src.GGUF.KeyValue("vision.image_mean"); mean.Valid() {
		projectorKV["clip.vision.image_mean"] = normalizeGGUFValue(mean.Any())
	}
	if std := src.GGUF.KeyValue("vision.image_std"); std.Valid() {
		projectorKV["clip.vision.image_std"] = normalizeGGUFValue(std.Any())
	}
...
func qwen25VLMropeSections(src *SourceModel) []int32 {
	for _, key := range []string{"qwen25vl.rope.dimension_sections", "qwen25vl.rope.mrope_section", "qwen2vl.rope.mrope_section"} {
		if keyValue := rawGGUFKeyValue(src.GGUF, key); keyValue.Valid() {
			ints := keyValue.Ints()
			if len(ints) > 0 {
				out := make([]int32, 4)
