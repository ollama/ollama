...
func skipMetadataArray(r io.Reader, byteOrder binary.ByteOrder, version uint32, arrayType uint32, count uint64) error {
	// Bound the element count before it feeds a multiply or a read loop so a
	// corrupt file cannot request a huge or overflowing skip.
	checkedCount, err := checkedLength(count, "array size", MaxArraySize)
	if err != nil {
		return err
	}
	...
	return discardMetadataBytes(r, int64(checkedCount)*size)
}
...
func readMetadataString(r io.Reader, byteOrder binary.ByteOrder, version uint32) (string, error) {
	...
	checkedLen, err := checkedLength(length, "string", MaxStringLength)
	if err != nil {
		return "", err
	}
	if checkedLen == 0 {
...
func skipMetadataString(r io.Reader, byteOrder binary.ByteOrder, _ uint32) error {
	...
	return discardMetadataBytes(r, int64(checkedLen))
}

func discardMetadataBytes(r io.Reader, n int64) error {
	if n < 0 {
		return fmt.Errorf("invalid gguf skip length %d", n)
	}
	if n == 0 {
		return nil
	}
