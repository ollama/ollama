...
func ScanMetadata(path string) (Metadata, error) {
	...
	var info Metadata
	byArchitecture := make(map[string]architectureMetadata)
	for range numKV {
		...
		if key == "general.architecture" {
			...
			applyArchitectureMetadata(&info, byArchitecture[value])
			continue
		}
		...
		if consumed, err := scanArchitectureMetadata(byArchitecture, key, r, byteOrder, version, valueType); err != nil {
			return Metadata{}, err
		} else if consumed {
			if info.Architecture != "" {
				applyArchitectureMetadata(&info, byArchitecture[info.Architecture])
			}
			continue
		}
...
func scanArchitectureMetadata(byArchitecture map[string]architectureMetadata, key string, r io.Reader, byteOrder binary.ByteOrder, version uint32, valueType uint32) (bool, error) {
	architecture, suffix, ok := strings.Cut(key, ".")
	if !ok {
		return false, nil
	}
	...
	case "context_length":
		...
		metadata.ContextLength = value
	case "embedding_length":
		value, err := readMetadataIntValue(r, byteOrder, version, valueType)
		if err != nil {
			return true, err
		}
		metadata.EmbeddingLength = value
	default:
		return false, nil
	}
	byArchitecture[architecture] = metadata
	return true, nil
}

func applyArchitectureMetadata(info *Metadata, metadata architectureMetadata) {
	info.ContextLength = metadata.ContextLength
	info.EmbeddingLength = metadata.EmbeddingLength
	info.HasAudio = metadata.HasAudio
	info.HasEmbedding = metadata.HasEmbedding
	info.HasVision = metadata.HasVision
}
