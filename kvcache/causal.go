...
// SkipK/SkipV (TurboQuant's compressed path) uses the same logic — the TQ
// encode path now takes the indexed-addressing route when locs are
// non-contiguous, so fragmented free cells (typical of sliding-window
// eviction) work directly.
...
func (c *Causal) Put(ctx ml.Context, key, value ml.Tensor) {
	// Tensor dims must be available for whichever side the cache actually
	// writes. SkipK / SkipV callers (TurboQuantCache, primarily) are allowed
	// to pass nil for the suppressed side; require non-nil for the side that
	// stays in this cache so the panic surfaces at the call site rather than
	// inside Reshape.
	if !c.SkipK && key == nil {
		panic(fmt.Errorf("Causal.Put: key is nil but SkipK is false (layer %d)", c.curLayer))
	}
	if !c.SkipV && value == nil {
		panic(fmt.Errorf("Causal.Put: value is nil but SkipV is false (layer %d)", c.curLayer))
	}
