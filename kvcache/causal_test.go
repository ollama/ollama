...
func TestCanResume(t *testing.T) {
	...
	cache.SetLayer(0)
	tensor := context.FromFloats([]float32{1, 2, 3, 4}, 1, 1, 4)
	cache.Put(context, tensor, tensor)
	...
	cache.SetLayer(0)
	tensor = context.FromFloats([]float32{5, 6}, 1, 1, 2)
	cache.Put(context, tensor, tensor)
...
func TestCanResumeSWAMem(t *testing.T) {
	...
	cache.SetLayer(0)
	tensor := context.FromFloats([]float32{1, 2, 3, 4, 5, 6}, 1, 1, 6)
	cache.Put(context, tensor, tensor)
	...
	cache.SetLayer(0)
	tensor = context.FromFloats([]float32{7, 8}, 1, 1, 2)
	cache.Put(context, tensor, tensor)
