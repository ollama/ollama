...
.PARAMETER ShowProgress
    Show installer progress for an app-initiated interactive upgrade. Normal
    installs and automatic app upgrades remain fully silent.
...
$CacheVersionFile = if ($env:OLLAMA_CACHE_VERSION_FILE) { $env:OLLAMA_CACHE_VERSION_FILE } else { "" }
...
if (($All -and $Minimal) -or
    ($All -and $Backends.Count -gt 0) -or
    ($Minimal -and $Backends.Count -gt 0)) {
    throw "OLLAMA_INSTALL_ALL, OLLAMA_INSTALL_MINIMAL, and OLLAMA_INSTALL_BACKENDS are mutually exclusive"
}
...
function Get-InstallerPayloadVersion {
    param([string]$InstallerPath)

    $productVersion = (Get-Item -LiteralPath $InstallerPath).VersionInfo.ProductVersion
    $productVersion = if ($productVersion) { $productVersion.Trim() } else { "" }
    if ($productVersion -notmatch '^[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$') {
        throw "Cached installer has invalid product version '$productVersion': $InstallerPath"
    }
    return $productVersion
}

function Write-InstallerCacheVersion {
    param([string]$Version)

    if (-not $CacheVersionFile) {
        return
    }
    if (-not [System.IO.Path]::IsPathRooted($CacheVersionFile)) {
        throw "OLLAMA_CACHE_VERSION_FILE must be an absolute path"
    }
    if ($Version -notmatch '^[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$') {
        throw "Cached installer has invalid version '$Version'"
    }

    $parentDir = Split-Path -Parent $CacheVersionFile
    if (-not (Test-Path -LiteralPath $parentDir -PathType Container)) {
        throw "OLLAMA_CACHE_VERSION_FILE parent directory does not exist: $parentDir"
    }

    $temporaryFile = "$CacheVersionFile.tmp"
    try {
        $encoding = New-Object System.Text.UTF8Encoding($false)
        [System.IO.File]::WriteAllText($temporaryFile, "$Version`n", $encoding)
        Move-Item -LiteralPath $temporaryFile -Destination $CacheVersionFile -Force
    } finally {
...
function New-TemporaryMsiSource {
    param([string]$MsiPath)

    $sourceDir = Join-Path (Get-TemporaryInstallerCacheRoot) ([guid]::NewGuid().ToString("N"))
    New-Item -ItemType Directory -Path $sourceDir -Force | Out-Null
    $temporaryPath = Join-Path $sourceDir (Split-Path $MsiPath -Leaf)

    try {
        # Windows Installer cannot always open packages below LOCALAPPDATA when
        # servicing them as SYSTEM. A hard link provides an accessible path
        # without duplicating large backend packages; copy across volumes.
        try {
            New-Item -ItemType HardLink -Path $temporaryPath -Target $MsiPath -ErrorAction Stop | Out-Null
        } catch {
            Copy-Item -LiteralPath $MsiPath -Destination $temporaryPath -Force -ErrorAction Stop
        }
    } catch {
...
function Install-Msi {
    ...
    $temporarySource = New-TemporaryMsiSource -MsiPath $MsiPath
    try {
        $uiMode = if ($ShowProgress) { "/passive" } else { "/quiet" }
        $msiArgs = @("/i", $temporarySource.Path, $uiMode, "/norestart")
        if ($TargetDir) {
        ...
        Remove-Item -LiteralPath $temporarySource.Root -Recurse -Force -ErrorAction SilentlyContinue
    }
}
...
function Test-MsiCoreInstall {
    try {
        ...
        throw "Unable to query Windows Installer products: $($_.Exception.Message)"
    }
}
...
function Get-InnoInstallerUrl {
    ...
    if ($Version) {
        $installerUrl = "${installerUrl}?version=$([uri]::EscapeDataString($Version))"
    }
    return $installerUrl
}
...
function Start-InnoInstaller {
    ...
    $arguments = @("/NORESTART", "/LOG=$logPath")
    if ($ShowProgress) {
        $arguments += @("/CLOSEAPPLICATIONS", "/FORCECLOSEAPPLICATIONS", "/SP", "/NOCANCEL", "/SILENT")
    } else {
...
function Invoke-InnoInstall {
    ...
    if ($All -or $Minimal -or $Backends.Count -gt 0) {
        throw "Backend selection options cannot be used with a legacy Inno Setup installation"
    }
    ...
    if ($CacheOnly) {
        ...
        if ($CacheVersionFile) {
            $cachedInstallerPath = Join-Path $cacheTarget.CacheDir $installerName
            $payloadVersion = Get-InstallerPayloadVersion -InstallerPath $cachedInstallerPath
            Write-InstallerCacheVersion -Version $payloadVersion
        }
        $cacheTargetCompleted = $true
        return
    }
    ...
    Write-Host "Install complete. You can now run 'ollama'."
        ...
        if (-not $cacheTargetCompleted -and -not $cacheTarget.UseExisting) {
            Remove-InstallCacheTarget -Target $cacheTarget
        }
    }
}
...
function Invoke-MsiInstall {
    ...
    if ($Version) {
        $coreUrl = "${coreUrl}?version=$([uri]::EscapeDataString($Version))"
    }
    ...
    $temporarySource = New-TemporaryMsiSource -MsiPath $coreMsiPath
    try {
        $tempExtract = Join-Path $temporarySource.Root "extract"
        ...
        $extractResult = Invoke-MsiExec -Arguments @("/a", $temporarySource.Path, "/qn", "TARGETDIR=$tempExtract") -LogPath $extractLogPath
        if ($extractResult.ExitCode -ne 0) {
            throw "Failed to extract packages.json from core MSI (exit code $($extractResult.ExitCode)). Log: $($extractResult.LogPath)"
        }
        ...
        if ($newManifestPath) {
            ...
            throw "packages.json not found in core MSI"
        }
    } finally {
        Remove-Item -LiteralPath $temporarySource.Root -Recurse -Force -ErrorAction SilentlyContinue
    }
    ...
    if ($All) {
        ...
        foreach ($b in $Backends) {
            ...
            if ($resolvedBackend) {
                ...
                throw "Requested backend '$b' is not available for $arch"
            }
        }
    ...
    if ($selectedBackends.Count -gt 0) {
        ...
        foreach ($backendName in $selectedBackends) {
            ...
            if (-not $pkg) {
                throw "Selected backend '$backendName' not found in package manifest"
            }
    ...
    if ($CacheOnly) {
        ...
        Write-InstallerCacheVersion -Version $newManifest.version
        $cacheTargetCompleted = $true
        return
    }
