...
function Invoke-Install {
    ...
    if ($CacheOnly) {
        ...
        if ($CacheVersionFile) {
            $payloadVersion = Get-InstallerPayloadVersion -InstallerPath $installer.Path
...
function Start-Installer {
    ...
    $installerArgs = @("/NORESTART", "/LOG=$installerLog")
    if ($ShowProgress) {
        $installerArgs += @(
            "/CLOSEAPPLICATIONS",
        ...
        $installerArgs += @("/VERYSILENT", "/SUPPRESSMSGBOXES")
    }
    if ($InstallDir) {
