...
if echo $PLATFORM | grep "," > /dev/null ; then
        ...
        tar c -C ./dist/linux_amd64 --exclude './lib/ollama/rocm*' --exclude './lib/ollama/.kpack' --exclude './lib/ollama/mlx*' --exclude './lib/ollama/include' . | zstd -9 -T0 >./dist/ollama-linux-amd64.tar.zst
        ( cd ./dist/linux_amd64 && tar c lib/ollama/rocm_v* lib/ollama/.kpack ) | zstd -9 -T0 >./dist/ollama-linux-amd64-rocm.tar.zst
        ...
        tar c -C ./dist/ --exclude 'lib/ollama/rocm*' --exclude 'lib/ollama/.kpack' --exclude 'lib/ollama/mlx*' --exclude 'lib/ollama/include' bin lib | zstd -9 -T0 >./dist/ollama-linux-amd64.tar.zst
        ( cd ./dist/ && tar c lib/ollama/rocm_v* lib/ollama/.kpack ) | zstd -9 -T0 >./dist/ollama-linux-amd64-rocm.tar.zst
