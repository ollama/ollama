...
_build_darwin() {
    ...
    for ARCH in $ARCHS; do
        ...
        if [ "$ARCH" = "amd64" ]; then
            ...
            cmake --build build/llama-server-cpu --target llama-server --target llama-quantize -j
...
_sign_darwin() {
    ...
    lipo -create -output dist/darwin/llama-quantize dist/darwin-*/lib/ollama/llama-quantize
