    ...
    $manifest = @{
        ...
        packages = @(
            ...
            @{
                ...
                sha256 = "aaa222bbb333"
                deps = "ollama-mlx-deps-13.0.48.msi"
...
function New-MockUpdateResponse {
    ...
    # The update API returns a URL to the installer
    # The Windows app updater converts the legacy API URL shape to ollama-core.msi.
    $response = @{
        url = "$DownloadBaseUrl/$Version/OllamaSetup.exe"
        version = $Version
    }
...
function Get-InnoSetupInstaller {
    ...
    if (-not $OutputDir) {
        $OutputDir = Join-Path $env:TEMP "ollama-inno-setup-test"
    }
    ...
    if (Test-Path $outputPath) {
        ...
        # Basic sanity check: file should be at least 50MB (real installer is ~100MB+)
...
function Test-MsiCachePopulated {
    param(
        [string[]]$ExpectedFiles = @("ollama-core.msi", "install.ps1"),
        [string]$CacheDir
    )
...
# Get the contents of the MSI cache directory.
function Get-MsiCacheContents {
    ...
    return Get-ChildItem -Path $CacheDir -File | Select-Object Name, Length, LastWriteTime
}
