...
.DESCRIPTION
    Integration tests (Tag: Integration) install and uninstall Ollama for real
    in the environment where they run. Invoke-WindowsInstallTests.ps1 uses a
    configured Windows Sandbox when available. Direct Pester runs still work
    on a clean host and abort if they detect an existing install.
...
.EXAMPLE
    # Uses Windows Sandbox when configured, otherwise uses the guarded host path.
    .\scripts\tests\Invoke-WindowsInstallTests.ps1 -Tag Integration

.EXAMPLE
    # Explicit direct-host run; destructive and requires no existing Ollama install.
    .\scripts\tests\Invoke-WindowsInstallTests.ps1 -Tag Integration -Isolation Host
#>
...
BeforeAll {
    ...
    $script:InstallerDownloadCacheDir = if ($env:OLLAMA_TEST_INSTALLER_CACHE_DIR) {
        $env:OLLAMA_TEST_INSTALLER_CACHE_DIR
    } else {
        Join-Path $script:RepoRoot ".cache\install-tests"
    }
    $script:DistInstaller = if ($env:OLLAMA_TEST_DIST_DIR) {
        Join-Path $env:OLLAMA_TEST_DIST_DIR "OllamaSetup.exe"
    } else {
        Join-Path $script:RepoRoot "dist\OllamaSetup.exe"
    }
    $script:OriginalLocalAppData = [Environment]::GetEnvironmentVariable("LOCALAPPDATA", "Process")
    $script:LatestInstallerVersion = $env:OLLAMA_TEST_LATEST_INSTALLER_VERSION
    $script:LatestInstallerUrl = if ($script:LatestInstallerVersion) {
        "https://github.com/ollama/ollama/releases/download/v$($script:LatestInstallerVersion)/OllamaSetup.exe"
    } else {
        "https://ollama.com/download/OllamaSetup.exe"
    }
    $script:LatestInstallerCacheName = if ($script:LatestInstallerVersion) {
        "OllamaSetup-$($script:LatestInstallerVersion).exe"
    } else {
        "OllamaSetup-latest.exe"
    }
    $script:PinnedUpgradeVersion = if ($env:OLLAMA_TEST_PINNED_UPGRADE_VERSION) {
        $env:OLLAMA_TEST_PINNED_UPGRADE_VERSION
    } else {
        "0.21.0"
    }
    ...
    function Get-IntegrationInstaller {
        if (-not $env:OLLAMA_TEST_IGNORE_DIST_INSTALLER -and (Test-Path -LiteralPath $script:DistInstaller)) {
        ...
        return Save-TestInstallerDownload -Url $script:LatestInstallerUrl -FileName $script:LatestInstallerCacheName
    }
    ...
    function Start-LocalHttpServer {
        ...
        if ($python) {
            ...
            $staticServer = Join-Path $script:RepoRoot "scripts\tests\static-server.ps1"
            $args = @(
                "-NoProfile",
    ...
    function Invoke-InstallScript {
        ...
        if ($timedOut -or $exitCode -ne 0) {
            if ($stdout) {
                ...
                Write-Host $(if ($stdout.Length -gt 8000) { $stdout.Substring($stdout.Length - 8000) } else { $stdout })
            }
            if ($stderr) {
                Write-Host "    install.ps1 stderr:" -ForegroundColor DarkGray
                Write-Host $(if ($stderr.Length -gt 8000) { $stderr.Substring($stderr.Length - 8000) } else { $stderr })
            }

            $installerLog = Join-Path $env:LOCALAPPDATA "Ollama\OllamaSetup.log"
            if ($env:OLLAMA_TEST_ARTIFACT_DIR -and (Test-Path -LiteralPath $installerLog -PathType Leaf)) {
                New-Item -ItemType Directory -Path $env:OLLAMA_TEST_ARTIFACT_DIR -Force | Out-Null
                $artifactLog = Join-Path $env:OLLAMA_TEST_ARTIFACT_DIR ("OllamaSetup-{0}.log" -f (Get-Date -Format "yyyyMMdd-HHmmss"))
                Copy-Item -LiteralPath $installerLog -Destination $artifactLog -Force
                Write-Host "    Installer log: $artifactLog" -ForegroundColor DarkGray
            }
        }
    ...
    function Invoke-TestOllamaUninstall {
        ...
        if ($env:OLLAMA_TEST_ARTIFACT_DIR) {
            New-Item -ItemType Directory -Path $env:OLLAMA_TEST_ARTIFACT_DIR -Force | Out-Null
            $uninstallLog = Join-Path $env:OLLAMA_TEST_ARTIFACT_DIR ("OllamaUninstall-{0}.log" -f (Get-Date -Format "yyyyMMdd-HHmmss"))
            $extraArgs += "/LOG=$uninstallLog"
        }
        $args = @($command.Arguments, (Join-TestProcessArguments $extraArgs)) -join " "
        $args = $args.Trim()
    ...
    function Reset-TestOllamaSandboxInstall {
        if ($env:OLLAMA_TEST_SANDBOX -ne "1") {
            throw "Sandbox cleanup may only run inside Windows Sandbox."
        }
        ...
        $installDir = Get-TestOllamaInstallDir
...
Describe "Windows install.ps1" -Tag Unit {
    ...
    Context "cache modes" {
    It "cache-only reports the signed payload version to a requested file" {
        $versionFile = Join-Path $TestDrive "stage\cache-version"
        New-Item -ItemType Directory -Path (Split-Path -Parent $versionFile) -Force | Out-Null
        Set-TestEnvironment @{
            OLLAMA_CACHE_ONLY = "1"
            OLLAMA_CACHE_VERSION_FILE = $versionFile
        }
        . $script:InstallScript

        Mock Get-RemoteETag { '"version-output-etag"' }
        Mock Invoke-Download {
            Set-Content -Path $OutFile -Value "payload" -Encoding ASCII
            return '"version-output-etag"'
        }
        Mock Test-Signature { $true }
        Mock Get-InstallerPayloadVersion { "0.21.0" }
        Mock Start-Process { throw "installer should not start in cache-only mode" }

        Invoke-Install

        (Get-Content -LiteralPath $versionFile -Raw).Trim() | Should -Be "0.21.0"
        Should -Invoke Get-InstallerPayloadVersion -Times 1 -Exactly
    ...
    It "install-cached shows progress only when explicitly requested" {
        Set-TestEnvironment @{ OLLAMA_INSTALL_CACHED = "1" }
        $cacheDir = Get-TestInstallerCacheDir '"cached-etag"'
        $installer = Join-Path $cacheDir "OllamaSetup.exe"
        New-Item -ItemType Directory -Path $cacheDir -Force | Out-Null
        Set-Content -Path $installer -Value "payload" -Encoding ASCII
        . $script:InstallScript -ShowProgress
        ...
        $script:StartedInstaller.FilePath | Should -Be $installer
        $script:StartedInstaller.ArgumentList | Should -Match "/SILENT"
        $script:StartedInstaller.ArgumentList | Should -Match "/CLOSEAPPLICATIONS"
        $script:StartedInstaller.ArgumentList | Should -Match "/FORCECLOSEAPPLICATIONS"
        $script:StartedInstaller.ArgumentList | Should -Not -Match "/VERYSILENT"
        $script:StartedInstaller.ArgumentList | Should -Not -Match "/SUPPRESSMSGBOXES"
        Test-Path $cacheDir | Should -Be $false
    }
...
Describe "Windows install.ps1 integration" -Tag Integration {
    ...
    It "upgrades from a pinned release through the two-phase app cache flow" {
        $signedInstaller = Require-RealInstallIntegration
        if (-not $signedInstaller) { return }
        ...
        try {
            ...
            $cacheVersionFile = Join-Path $TestDrive "app-stage\cache-version"
            New-Item -ItemType Directory -Path (Split-Path -Parent $cacheVersionFile) -Force | Out-Null
            $server = Start-LocalHttpServer -InstallerPath $signedInstaller -ETag $etag
            if (-not $server) { return }
            $installScript = New-TestInstallScript -DownloadBaseUrl $server.DownloadBaseUrl

            $result = Invoke-InstallScript -InstallScriptPath $installScript -EnvVars @{
                OLLAMA_CACHE_ONLY = "1"
                OLLAMA_CACHE_VERSION_FILE = $cacheVersionFile
            }
            $result.ExitCode | Should -Be 0
            ...
            (Get-Content -LiteralPath $cacheVersionFile -Raw).Trim() | Should -Be $expectedVersion
