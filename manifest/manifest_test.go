...
func TestWriteManifestDataKeepsManifestBlobDuringConcurrentPrune(t *testing.T) {
	t.Setenv("OLLAMA_MODELS", t.TempDir())
	...
	oldHook := testBeforeManifestLinkReplace
	testBeforeManifestLinkReplace = func() {
		close(linkStarted)
		<-releaseLink
	}
	t.Cleanup(func() {
		testBeforeManifestLinkReplace = oldHook
	})

	writeDone := make(chan error, 1)
	go func() {
		writeDone <- WriteManifestData(name, data)
	}()

	select {
	case <-linkStarted:
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for manifest link")
	}
