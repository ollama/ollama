...
func TestV2ManifestPathEscapesHostPort(t *testing.T) {
	...
	if len(parts) != 4 {
		t.Fatalf("manifest path %q has %d parts, want 4", rel, len(parts))
	}
	if got, want := parts[0], "127.0.0.1%3A12345"; got != want {
		t.Fatalf("encoded host path part = %q, want %q", got, want)
	}

	gotData, err := ReadManifestData(name)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(gotData, data) {
		t.Fatal("read manifest data differs from written manifest data")
	}

	manifests, err := Manifests(false)
	if err != nil {
		t.Fatal(err)
	}
	if _, ok := manifests[name]; !ok {
		t.Fatalf("manifest %s was not enumerated", name)
	}
}
...
func TestManifestStoreLockKeepsManifestBlobDuringConcurrentPrune(t *testing.T) {
	...
	if got, err := writeManifestBlob(data); err != nil {
		t.Fatal(err)
	} else if !SameDigest(got, digest) {
		t.Fatalf("manifest blob digest = %q, want %q", got, digest)
	}

	pruneDone := make(chan error, 1)
	go func() {
		_, err := RemoveUnreferencedBlobs(digest)
		pruneDone <- err
	}()

	select {
	case err := <-pruneDone:
		t.Fatalf("prune completed before manifest link was visible: %v", err)
	...
	if _, err := os.Stat(blobPath); err != nil {
		t.Fatalf("manifest blob was pruned after link: %v", err)
	}
	if _, err := ParseNamedManifest(name); err != nil {
		t.Fatalf("manifest was not readable after concurrent prune: %v", err)
	}
}
...
func TestWriteManifestBlobDoesNotRemoveDestinationOnRenameFailure(t *testing.T) {
	...
	if _, err := writeManifestBlob(data); err == nil {
		t.Fatal("writeManifestBlob succeeded with an existing directory at the blob path")
	}
	...
	if !fi.IsDir() {
		t.Fatalf("blob path was replaced; mode = %s, want directory", fi.Mode())
	}
}
...
func TestParseNamedManifestUsesNamedPathModTime(t *testing.T) {
	...
	manifestPath, err := ResolvePathForName(name)
	if err != nil {
		t.Fatal(err)
	}
	namedInfo, err := os.Lstat(manifestPath)
	if err != nil {
		t.Fatal(err)
	}

	mf, err := ParseNamedManifest(name)
	if err != nil {
		t.Fatal(err)
	}
	blobPath, err := BlobsPath(mf.BlobDigest())
	if err != nil {
		t.Fatal(err)
	}
	oldBlobTime := namedInfo.ModTime().Add(-time.Hour)
	if err := os.Chtimes(blobPath, oldBlobTime, oldBlobTime); err != nil {
		t.Fatal(err)
	}
	...
	if got := mf.FileInfo().ModTime(); !got.Equal(namedInfo.ModTime()) {
		t.Fatalf("manifest mod time = %s, want named path %s", got, namedInfo.ModTime())
	}
}

func TestDigestReference(t *testing.T) {
	hex := strings.Repeat("a", 64)
	digest, ok := DigestReference("sha256-" + hex)
	if !ok {
		t.Fatal("DigestReference() did not parse valid reference")
	}
	if digest != "sha256:"+hex {
		t.Fatalf("digest = %q, want sha256:%s", digest, hex)
	}
	...
	for _, ref := range []string{
		...
		if _, ok := DigestReference(ref); ok {
			t.Fatalf("DigestReference(%q) parsed unexpectedly", ref)
		}
	}
}
...
func TestParseNamedManifestByDigestReference(t *testing.T) {
	...
	digestRef := strings.Replace(digest, ":", "-", 1)

	got, err := ParseNamedManifest(model.ParseName(digestRef))
	if err != nil {
		t.Fatal(err)
	}
	if got.BlobDigest() != digest {
		t.Fatalf("BlobDigest() = %q, want %q", got.BlobDigest(), digest)
	}
	if got.Runner != RunnerLlamaCPP {
		t.Fatalf("Runner = %q, want %q", got.Runner, RunnerLlamaCPP)
	}
}
...
func TestParseManifestListByDigestReference(t *testing.T) {
	...
	childRef, err := NewManifestReference(childDigest, RunnerGGML, FormatGGUF)
	if err != nil {
		t.Fatal(err)
	}

	parentData := createManifestListData(t, childRef)
	parentDigest := writeManifestBlobForTest(t, parentData)
	parentRef := strings.Replace(parentDigest, ":", "-", 1)

	raw, err := ReadManifestData(model.ParseName(parentRef))
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(raw, parentData) {
		t.Fatal("ReadManifestData() returned different parent manifest bytes")
	}

	got, err := ParseNamedManifestForRunner(model.ParseName(parentRef), RunnerGGML)
	if err != nil {
		t.Fatal(err)
	}
	if got.BlobDigest() != parentDigest {
		t.Fatalf("BlobDigest() = %q, want parent %q", got.BlobDigest(), parentDigest)
	}
	if got.SelectedDigest() != strings.TrimPrefix(childDigest, "sha256:") {
		t.Fatalf("SelectedDigest() = %q, want child %q", got.SelectedDigest(), strings.TrimPrefix(childDigest, "sha256:"))
	}
	if got.Runner != RunnerGGML {
		t.Fatalf("Runner = %q, want %q", got.Runner, RunnerGGML)
	}
}
...
func TestRemoveNamedByDigestReference(t *testing.T) {
	...
	digestRef := strings.Replace(m.BlobDigest(), ":", "-", 1)

	if _, err := RemoveNamed(model.ParseName(digestRef)); err != nil {
		t.Fatal(err)
	}
	if _, err := ResolvePathForName(name); !os.IsNotExist(err) {
		t.Fatalf("ResolvePathForName() error = %v, want not exist", err)
	}
	...
	if _, err := os.Stat(blobPath); !os.IsNotExist(err) {
		t.Fatalf("manifest blob still exists, stat error = %v", err)
	}
}
...
func TestSelectManifestReferenceDoesNotResolveBlob(t *testing.T) {
	...
	child, err = SelectManifestReferenceForRunner([]Manifest{ref}, RunnerGGML)
	if err != nil {
		t.Fatal(err)
	}
	if got := child.BlobDigest(); got != "sha256:"+strings.Repeat("a", 64) {
...
func TestParseNamedManifestResolvesManifestList(t *testing.T) {
	...
	if !bytes.Equal(selected, llamacppData) {
...
func TestRemoveNamedTracksManifestListChildBlobs(t *testing.T) {
	...
	if _, err := RemoveNamed(nameA); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := RemoveNamed(nameB); err != nil {
		t.Fatal(err)
	}
...
func TestRemoveNamedDigestRemovesManifestListChild(t *testing.T) {
	...
	ggmlRef, err := NewManifestReference(ggmlDigest, RunnerGGML, FormatGGUF)
	if err != nil {
		t.Fatal(err)
	}
	llamacppRef, err := NewManifestReference(llamacppDigest, RunnerLlamaCPP, FormatGGUF)
	if err != nil {
		t.Fatal(err)
	}

	name := model.ParseName("example")
	if err := WriteManifestData(name, createManifestListData(t, ggmlRef, llamacppRef)); err != nil {
		t.Fatal(err)
	}

	digestName := model.ParseName(strings.Replace(llamacppDigest, ":", "-", 1))
	if _, err := RemoveNamed(digestName); err != nil {
		t.Fatal(err)
	}
	...
	if len(parent.Manifests) != 1 {
		t.Fatalf("manifest child count = %d, want 1", len(parent.Manifests))
	}
	if parent.Manifests[0].Runner != RunnerGGML {
		t.Fatalf("remaining runner = %q, want %q", parent.Manifests[0].Runner, RunnerGGML)
	}

	childPath, err := BlobsPath(llamacppDigest)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(childPath); !os.IsNotExist(err) {
		t.Fatalf("removed child manifest blob still exists: %v", err)
	}
}
...
func TestTensorLayers(t *testing.T) {
	m := &Manifest{Layers: []Layer{
		{MediaType: MediaTypeImageTensor, Size: 1000},
		{MediaType: MediaTypeImageJSON, Size: 500},
		{MediaType: MediaTypeImageTensor, Size: 2000},
	}}

	got := m.TensorLayers()
	if len(got) != 2 || got[0].Size != 1000 || got[1].Size != 2000 {
		t.Fatalf("TensorLayers() = %+v, want the two tensor layers", got)
	}
}
...
func TestReadConfig(t *testing.T) {
	...
	layer.Name = "config.json"
	m := &Manifest{Layers: []Layer{layer}}

	var cfg struct {
		ModelType string `json:"model_type"`
	}
	if err := m.ReadConfigJSON("config.json", &cfg); err != nil {
		t.Fatal(err)
	}
	if cfg.ModelType != "test" {
		t.Fatalf("model_type = %q, want %q", cfg.ModelType, "test")
	}
	if _, err := m.ReadConfig("missing.json"); err == nil {
		t.Fatal("ReadConfig(missing.json) returned nil error")
	}
}
