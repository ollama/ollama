...
func V2PathForName(n model.Name) (string, error) {
	...
	return filepath.Join(manifests, v2NameFilepath(canonicalV2Name(n))), nil
}
...
func resolveManifestPath(n model.Name) (string, string, error) {
	...
	for _, legacyName := range legacyNameCandidates(n) {
		if !legacyManifestPathSupported(legacyName) {
			continue
		}
...
func v2NameFilepath(n model.Name) string {
	if !n.IsFullyQualified() {
		panic("illegal attempt to get filepath of invalid name")
	}
	return filepath.Join(
		encodeV2ManifestHost(n.Host),
		n.Namespace,
		n.Model,
		n.Tag,
	)
...
func parseStoredManifestFilepath(s string) model.Name {
	...
	n := model.Name{
		Host:      decodeV2ManifestHost(parts[0]),
		Namespace: parts[1],
		Model:     parts[2],
		Tag:       parts[3],
	}
...
func encodeV2ManifestHost(host string) string {
	return strings.ReplaceAll(host, ":", "%3A")
}

func decodeV2ManifestHost(host string) string {
	return strings.NewReplacer("%3A", ":", "%3a", ":").Replace(host)
}
...
func legacyManifestPathSupported(n model.Name) bool {
	// Windows cannot represent host:port registry names in the legacy manifest tree.
	return runtime.GOOS != "windows" || !strings.Contains(n.Host, ":")
}
