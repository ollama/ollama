...
public:
    ...
    // Pages are allocated in layer-major order: pages[layers][blocks]
    bool get_kv_ptrs(int seq_id, int layer, int token_pos,
        ...
        int pages_per_layer = (int)it->second.size() / num_heads; // simplified: flat layout
        int block_in_layer = token_pos / KV_PAGE_SIZE;
        int global_idx = layer * (max_tokens_per_seq / KV_PAGE_SIZE) + block_in_layer;

        if (global_idx >= (int)it->second.size() || global_idx < 0) return false;
...
private:
    ...
    KVPage* find_free_page() {
        for (auto& p : pages) {
            if (p.seq_id < 0 && !p.locked.load()) return &p;
        }
        return nullptr;
    }
    ...
    KVPage* evict_oldest_unlocked() {
        ...
        for (auto& p : pages) {
            if (!p.locked.load() && p.seq_id >= 0 && p.last_access < oldest_time) {
                oldest = &p;
...
struct RoPECacheDevice {
    ...
    RoPECacheDevice(int dim=128, float base=10000.0f, float scale=1.0f,
        ...
        // Free host-side data after device upload
        delete[] cos_host;
        cos_host = nullptr;
        delete[] sin_host;
        sin_host = nullptr;
    ...
    ~RoPECacheDevice() {
        if (cos_host) delete[] cos_host;
        if (sin_host) delete[] sin_host;
        if (cos_dev) hipFree(cos_dev);
        if (sin_dev) hipFree(sin_dev);
    }
};
...
__global__ void quantize_q8_0_kernel(
    ...
    // Warp reduce max (start from 32 for wave32/wave64 compatibility)
...
__global__ void fused_q8_0_attention_kernel(
    ...
    // Warp reduce (start from 32 for wave32/wave64 compatibility)
    for (int offset = 32; offset > 0; offset >>= 1) {
