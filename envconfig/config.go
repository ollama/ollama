...
// StreamHeartbeatMS returns the default streaming heartbeat interval in
// milliseconds. It can be configured via OLLAMA_STREAM_HEARTBEAT_MS.
// Values <= 0 disable heartbeats.
// Default is 10000 (10 seconds).
func StreamHeartbeatMS() int {
	const defaultValue = 10000

	if s := Var("OLLAMA_STREAM_HEARTBEAT_MS"); s != "" {
		n, err := strconv.ParseInt(s, 10, 64)
		if err != nil {
			slog.Warn("invalid environment variable, using default", "key", "OLLAMA_STREAM_HEARTBEAT_MS", "value", s, "default", defaultValue)
			return defaultValue
		}
...
func AsMap() map[string]EnvVar {
	ret := map[string]EnvVar{
		...
		"OLLAMA_STREAM_HEARTBEAT_MS": {"OLLAMA_STREAM_HEARTBEAT_MS", StreamHeartbeatMS(), "Default streaming heartbeat interval in milliseconds (default \"10000\", <=0 disables)"},
